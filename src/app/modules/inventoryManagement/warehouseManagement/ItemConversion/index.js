...
export default function ItemConversionLanding() {
    ...
    const [currentItemRate, getItemCurrentRate, itemCurrentRateLoader] = useAxiosGet()
    ...
    const saveHandler = (values, cb) => {
        ...
        for (const row of rowDto) {
            ...
            if (parseFloat(row.convertedQuantity) > parseFloat(row.itemAvailableStock)) {
                return toast.warn("Converted Quantity cannot be greater than Item Stock.");
            }
        }
        ...
        onCreateCoversion(`/wms/ItemConversion/CreateItemConversion`, payload[0], cb, true)
    };


    const handleAddItem = async (values, setFieldValue) => {
        if (rowDto.length > 0) {
            return toast.warn("You can't add more than one item conversion at a time.");
        }
        ...
        try {
            ...
            const [stockResponse, rateResponse] = await Promise.all([
                axios.get(`/wms/InventoryTransaction/sprRuningQty?businessUnitId=${buId
                    }&whId=${values?.warehouse?.value
                    }&itemId=${values.item?.value}`),
                axios.get(`/wms/InventoryLoan/GetItemRate?ItemId=${values.item?.value}&BusinessUnitId=${buId}`)
            ]);
                            ...
                            <div className="form-group  global-form row">
                                ...
                                <div className="col-lg-3" style={{ marginTop: 20 }}>
                                    <button
                                        ...
                                        disabled={isAdding || !values?.plant || !values?.warehouse || !values?.item || !values?.convertedPlant || !values?.convertedWarehouse || !values?.convertedItem || !values?.convertedQuantity}
                                    >
                                        Add
                                    </button>
                                </div>
                                    ...
                                    <tbody>
                                        {rowDto?.map((item, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className="text-center">{item?.itemAvailableStock}</td>
                                                <td className="text-center">{item?.itemRate}</td>
                                                <td>{item?.convertedItem}</td>
                                                <td className="text-center">{item?.convertedItemCode}</td>
                                                <td className="text-center">{item?.convertedUomName}</td>
                                                <td><InputField value={item?.convertedQuantity || ""} type="number" onChange={(e) => {
                                                    // converted quantity can not be negative
                                                    if (e.target.value <= 0) return;
                                                    const data = [...rowDto];
                                                ...
                                                <td className="text-center">
                                                    <span onClick={() => {
                                                        const filterData = rowDto.filter((itm, indx) => indx !== index);
