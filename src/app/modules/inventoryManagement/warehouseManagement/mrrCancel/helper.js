...
export const getReportForCanceledMRR = async (transactionId, businessUnitId, setter) => {
  try {
    const res = await Axios.get(
      `/wms/InventoryTransaction/GetInventoryTransectionMRRCancelReportViewById?transactionId=${transactionId}&businessUnitId=${businessUnitId}`
    );
    setter(res?.data[0]);
  } catch (error) {}
};
