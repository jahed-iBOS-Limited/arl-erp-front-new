...
export default function ItemWiseSerialCreate() {
  ...
  useEffect(() => {
    setSbuDDL(`/costmgmt/SBU/GetSBUListDDL?AccountId=${profileData?.accountId}&BusinessUnitId=${selectedBusinessUnit.value}&Status=true`)
    setPlantDDL(`/wms/BusinessUnitPlant/GetOrganizationalUnitUserPermission?UserId=${profileData?.userId}&AccId=${profileData?.accountId}&BusinessUnitId=${selectedBusinessUnit.value}&OrgUnitTypeId=7`)
                  ...
                  <div className="row">                   
                    ...
                    <SearchAsyncSelect
                      ...
                      handleChange={(valueOption) => {
                        ...
                        getMrrNoDDL(`/wms/ItemWiseSerialUpdate/GetMRRListDDL?AccountId=${profileData?.accountId}&BusinessUnitId=${selectedBusinessUnit?.value}&SBUId=${values?.sbu?.value}&PlantId=${values?.plant?.value}&WarehouseId=${values?.warehouse?.value}&ReferenceId=${valueOption?.value}&ReferenceCode=${valueOption?.label}`)
                        setFieldValue("mrrNo", "")
                      }}
                      loadOptions={(v) => {
                  ...
                  <div className="col-lg-2">
                      <NewSelect
                        ...
                        onChange={(valueOption) => {
                          ...
                          getRowData(`/wms/ItemWiseSerialUpdate/GetMRRItemListByMRRId?MRRId=${valueOption?.value}`)                          
                        }}
                        errors={errors}
                      />
                    </div>
                        ...
                        {rowData?.length > 0 &&
                          rowData?.map((item, index) => (
                            <tr key={index}>
                              ...
                              <td className="">{values?.mrrNo?.businessPartnerName}</td>
                              <td className="text-center">{item?.transactionQuantity}</td>
                            </tr>
                          ))}
                      </tbody>
                    </table>
