...
const ItemInformation = ({
  ...
  const itemSerialList = Array.isArray(rowDto[currentRowIndex]?.serialList)
    ? rowDto[currentRowIndex]?.serialList
    : []; //from inventory receive page
  ...
  const onRowChange = ({ value, key, index }) => {
    const updatedList = getUpdatedSecondLevelList(
      currentRowIndex,
      index,
      key,
      value
  ...
  const handleAllCheck = (value) => {
    ...
    if (isEdit) {
      const updatedChildList = exactClickedRow?.scannedItemSerialList.map(
        (item) => {
          return {
            ...item,
  ...
  const getChecked = () => {
    ...
    return (
      childList?.length > 0 &&
      childList?.every(
        (item) => item?.isCheck || item?.isItWithCurrentPurchaseOrder
      )
    );
  };
                    ...
                    <tr
                      ...
                      style={{
                        backgroundColor: (() => {
                          ...
                          const diffInDays =
                            (currentDate - mrrDate) / (1000 * 60 * 60 * 24);
                    ...
                    >
                      <td style={{ width: '5px' }}>
                        <input
                          ...
                          name="isCheck"
                          checked={
                            item?.isCheck || item?.isItWithCurrentPurchaseOrder
                          }
                          onChange={(e) => {
