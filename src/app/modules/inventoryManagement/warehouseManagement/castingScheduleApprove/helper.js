...
export const ApproveCastingScheduleData = async (payload, setLoading, cb) => {
  setLoading(true);
  try {
    const res = await axios.put(
      `/oms/CastingSchedule/ApproveCastingSchedule`,
      payload
    );
    toast.success(res?.data?.message);
    cb && cb();
...
export const RejectCastingScheduleData = async (payload, setLoading, cb) => {
  setLoading(true);
  try {
    const res = await axios.put(
      `/oms/CastingSchedule/CancelCastingScheduleApprove`,
      payload
    );
    toast.success(res?.data?.message);
    cb && cb();
...
export const getCastingEntryListApprovePage = async (
  buId,
  values,
  pageNo,
  pageSize,
  ...
  try {
    const res = await axios.get(
      `/oms/CastingSchedule/GetCastingScheduleApprovalRequest?FromDate=${values?.fromDate}&ToDate=${values?.toDate}&BusinessUnitId=${buId}&ShippingpointId=${values?.shipPoint?.value}&PageSize=${pageSize}&PageNo=${pageNo}`
    );

    // const dataset = [];
    // for (let item of res?.data?.data) {
    //   for (let nestedItem of item?.list) {
    //     dataset.push({
    //       ...nestedItem,
    //       ...item,
    //       list: undefined,
