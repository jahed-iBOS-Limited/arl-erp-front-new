...
export default function InventoryLoanLandingNew() {
    ...
    const getLandingData = (values, pageNo, pageSize) => {
        if (profileData?.accountId && selectedBusinessUnit?.value && values?.partner) {
            getRowData(`/wms/InventoryLoan/GetInvItemloanListPagination?AccountId=${profileData?.accountId}&BusinessUnitId=${selectedBusinessUnit?.value}&fromDate=${values?.fromDate}&toDate=${values?.toDate}&partnerId=${values?.partner?.value}&PageNo=${pageNo}&PageSize=${pageSize}&viewOrder=desc`)
        }
    };
                                        ...
                                        <tbody>
                                            {rowData?.data?.map((item, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td>
                                                        <div className="text-center">{item?.strInventoryTransactionCode}</div>
                                                    </td>
                                                    <td>
