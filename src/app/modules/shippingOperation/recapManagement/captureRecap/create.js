...
export default function RecapCreate() {
  ...
  useEffect(() => {
    if (viewId) {
      getViewData(
        `${imarineBaseUrl}/domain/VesselNomination/GetByIdVesselNomination?VesselNominationId=${viewId}`,
        (res) => {
          const data = {
            voyageType: res?.intVoyageTypeId
              ? { value: res.intVoyageTypeId, label: res.strVoyageType }
              : '',
            shipType: res?.intShipTyeId
              ? { value: res.intShipTyeId, label: res.strShipType }
              : '',
            vesselName: res?.intVesselId
              ? { value: res.intVesselId, label: res.strNameOfVessel }
              : '',
            deliveryPort: res?.strPlaceOfDelivery
              ? { label: res.strPlaceOfDelivery }
              : '',
            ...
            brokerName: res?.intBrokerId
              ? { value: res.intBrokerId, label: res.strBrokerName }
              : '',
...
`);
  ...
  const saveHandler = async (values, cb) => {
    ...
    const hasNominationCargos = chartererList.every((charterer) => {
      return (
        charterer.nominationCargosList &&
        charterer.nominationCargosList.length > 0
      );
    });
            ...
            {isShowModal && (
              ...
              >
                <NominationCargosList
                  nominationCargosList={singleData?.nominationCargosList}
                  isModalView={true}
                />
              </IViewModal>
            )}
