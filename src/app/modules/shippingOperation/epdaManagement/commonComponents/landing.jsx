...
export default function CommonLoadAndDischargePortLanding({ featureName }) {
  ...
  const getLandingData = (values, pageNo, pageSize, searchValue = '') => {
    ...
    let controllerName =
      featureName === 'Load Port'
        ? 'GetEpdaAndPortInfoLanding'
        : featureName === 'Discharge Port'
          ? 'GetFromEpdaAndDischargePortInfoLanding'
          : '';

    getGridData(
      `${imarineBaseUrl}/domain/VesselNomination/${controllerName}?BusinessUnitId=${0}&FromDate=${
        values?.fromDate || _previousDate()
      }&ToDate=${values?.toDate || _todayDate()}&pageNumber=${
        pageNo || 1
      }&pageSize=${
        pageSize || 600
