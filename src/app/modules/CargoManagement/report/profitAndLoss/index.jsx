...
function ProfitAndLoss() {
  ...
  const commonReportDataSet = (reportData, values) => {
    ...
    const collecttionBillingDatasFilter =
      reportData?.collecttionBillingDatas?.filter((item) => {
        return bookingRequestId
          ? item?.bookingRequestId === bookingRequestId
          : true;
      });
    ...
    const collecttionHeadOfChargeWiseGroup =
      collecttionBillingDatasFilter.reduce((acc, curr) => {
        let { headOfChargeId, collectionActualAmount, collectionDummyAmount } =
          curr;
