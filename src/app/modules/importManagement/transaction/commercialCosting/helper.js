...
export const getCommercialCostingLanding = async ({
  ...
  try {
    ...
    const res = await axios.get(
      `/imp/AllCharge/GetCommercialCosting?accountId=${accId}&businessUnitId=${buId}${dteFromDate}${dteToDate}&isCompleted=${isCompleted}&bankId=${
        bank?.value || 0
    ...
    getExcelData(
      `/imp/AllCharge/GetCommercialCosting?accountId=${accId}&businessUnitId=${buId}${dteFromDate}${dteToDate}&isCompleted=${isCompleted}&bankId=${
        bank?.value || 0
      }&PageNo=${pageNo}&PageSize=${res?.data?.totalCount}`
    );
  } catch (error) {
    setLoading(false);
    toast.error(error?.response?.data?.message);
