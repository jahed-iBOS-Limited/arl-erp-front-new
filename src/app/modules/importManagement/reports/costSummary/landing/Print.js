    ...
    <>
      <div ref={printRef}>
        ...
        <div className="global-form text-center mt-2">
          <h6 className="">
            {values?.shipment?.value ? <span className="text-primary font-weight-bold cursor-pointer mr-2"
              style={{ textDecoration: 'underline' }} onClick={
                () => setIsShowShipmentQuantity(true)
              }>{values?.shipment?.label}</span> : "LC Wise Total "} Cost
            Information
          </h6>
          {rowDto?.length > 0 && (
            <>
              {/* <b className="ml-5">PO No : {values?.poLc?.poNumber}</b> */}
              <b className="ml-5">PO No : <span
              ...
              <b className="ml-5">Supplier : {values?.shipment?.supplier ? values?.shipment?.supplier : values?.poLc?.supplier}</b>{" "}
              <b className="ml-5">
                Total Value : {_formatMoney(values?.shipment?.value ? values?.shipment?.shipmentWiseAmount : values?.poLc?.poAmount)} USD
              </b>{" "}
            </>
          )}
        </div>
            ...
            <tbody>
                ...
                rowDto?.map((item, index) => {
                  return item?.section === 1 ? (
                    <tr key={index}>
                      ...
                      <td className="text-right">
                        {
                          _formatMoney((item?.numBookedAmount - item?.numActualAmount).toFixed(4)) || 0
                        }
                      </td>
                    </tr>
                  ) : (
              ...
              {rowDto?.length > 0 && (
                <tr className="font-weight-bold">
                  ...
                  <td className="text-right">
                    {
                      _formatMoney(rowDto?.reduce((acc, curr) => curr?.section === 1 ? acc + (curr?.numActualAmount || 0) : acc, 0))
                    }
                  </td>
                  <td className="text-right">
                    {
                      _formatMoney(rowDto?.reduce((acc, curr) => curr?.section === 1 ? (acc + (curr?.numBookedAmount - curr?.numActualAmount || 0)) : acc, 0).toFixed(4))
                    }
                  </td>
                </tr>
              )}
