...
export default function BareboatChartererConfigCreateEdit() {
      ...
      if (id) {
         getSingleData(
            ...
            res => {
               const data = {
                  categoryType: {
                     value: res?.intCategoryTypeId === 1 ? 1 : 2,
                     label:
   ...
   // /procurement/PurchaseOrder/TransferPoBusinessUnit?UnitId=102
   ...
   const saveHandler = (values, cb) => {
      const payload = {
         ...
         intTransferRevenueElementId:
            values?.transferRevenueElement?.value || 0,
      };
      console.log('edit');
   ...
   // const modifySupplierDDL = supplierDDL?.filter(
   //    itm =>
   //       itm?.value === 41845 || itm?.value === 69484 || itm?.value === 41811
   // );
   ...
   return (
      ...
      >
         {({
            ...
            <>
               {(singleLoading || supplierLoading || baseTypeLoading || vesselAssetLoading || profitCenterDDLloader
               
               || costCenterDDLloader || costElementDDLloader || transferBusinessUnitDDLloader || transferProfitCenterDDLloader
               || transferRevenueCenterDDLloader || transferRevenueElementDDLloader) && (
                  <Loading />
               )}
               <IForm
                  title={
                     id
                        ? 'Edit Bareboat Management And Insurance'
                        : 'Create Bareboat Management And Insurance'
                  }
