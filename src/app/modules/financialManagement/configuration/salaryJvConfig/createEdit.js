...
export default function SalaryJvConfigCreateEdit() {
    ...
    const [profitCenterDDl, getProfitCenterDDL, profitCenterDDLloader] = useAxiosGet()
    const [debitGeneralLedgerDDL, getDebitGeneralLedgerDDL, debitGeneralLedgerDDLloader] = useAxiosGet()
    const [debitSubGeneralLedgerDDL, getDebitSubGeneralLedgerDDL, debitSubGeneralLedgerDDLloader, setDebitSubGeneralLedgerDDL] = useAxiosGet()
    const [creditGeneralLedgerDDL, , , setCreditGeneralLedgerDDL] = useAxiosGet()
    const [creditSubGeneralLedgerDDL, getCreditSubGeneralLedgerDDL, creditSubGeneralLedgerDDLloader, setCreditSubGeneralLedgerDDL] = useAxiosGet()
    const [salaryComponentDDL, getSalaryComponentDDL, salaryComponentDDLloader, setSalaryComponentDDL] = useAxiosGet()
    ...
    const saveHandler = (values, cb) => {
        if (tableData?.length < 1) return toast.warn("Please add at least one item")
        createSalaryJvConfig(`/fino/AdjustmentJournal/CreateNUpdateDebitCreditGlConfig?BusinessUnitId=${selectedBusinessUnit?.value}`, tableData, cb, true)
    };
    ...
    const addHandler = (values) => {
        const isExist = tableData?.find((item) => {
            return (
                ...
                item?.intDebitBusinessTransactionId === values?.debitSubGeneralLedger?.value &&
                item?.intCreditGlid === values?.creditGeneralLedger?.value &&
                item?.intCreditBusinessTransactionId === values?.creditSubGeneralLedger?.value &&
    ...
    useEffect(() => {
        if (location?.state?.isEdit === true) {
            getOldConfigData(`/fino/AdjustmentJournal/GetAllDebitCreditGLConfig?BusinessUnitId=${selectedBusinessUnit?.value}`,
                (data) => {
        ...
        getSbuDDL(`/costmgmt/SBU/GetSBUListDDL?AccountId=${profileData?.accountId}&BusinessUnitId=${selectedBusinessUnit?.value}&Status=true`)
        getDebitGeneralLedgerDDL(`/fino/CommonFino/GetGeneralLedgerListScheduleView`, (data) => setCreditGeneralLedgerDDL(data))
        getSalaryComponentDDL(`/fino/AdjustmentJournal/GetSalaryComponentDDL`, (data) => {
    ...
    return (
        ...
        >
            {({
                ...
                <>
                    {(costCenterDDLloader || sbuDDLloader || CostElementDDLloader || profitCenterDDLloader || debitGeneralLedgerDDLloader || debitSubGeneralLedgerDDLloader || salaryComponentDDLloader || creditSubGeneralLedgerDDLloader || createSalaryJvConfigLoader || oldConfigDataLoader) && <Loading />}
                    <IForm title={
                        id ? "Edit Salary JV Configuration" : "Create Salary JV Configuration"
                    } getProps={setObjprops} submitBtnText={
                            ...
                            <div className="form-group  global-form row">
                                <div className="col-lg-3">
                                    <NewSelect
                                        ...
                                        onChange={(valueOption) => {
                                            ...
                                            getCostCenterDDL(`/costmgmt/CostCenter/GetCostCenterDDL?AccountId=${profileData?.accountId
                                                }&BusinessUnitId=${selectedBusinessUnit?.value
                                                }&SBUId=${valueOption?.value
                                                }`)
                                        }}
                                        placeholder="SBU"
                                        errors={errors}
                                    ...
                                    <ISelect
                                        ...
                                        onChange={(valueOption) => {
                                            if (valueOption) {
                                                ...
                                                getProfitCenterDDL(`/costmgmt/ProfitCenter/GetProfitcenterDDLByCostCenterId?costCenterId=${valueOption?.value
                                                    }&businessUnitId=${selectedBusinessUnit?.value
                                                    }`)
                                            } else {
                                ...
                                <div className="col-lg-3">
                                    <NewSelect
                                        ...
                                        onChange={(valueOption) => {
                                            ...
                                            getDebitSubGeneralLedgerDDL(`/fino/FinanceCommonDDL/GetBusinessTransactionDDL?accountId=${profileData?.accountId
                                                }&businessUnitId=${selectedBusinessUnit?.value
                                                }&generalLedgerId=${valueOption?.value
                                                }`, (data) => {
                                                    const newData = data?.map((item) => ({
                                ...
                                <div className="col-lg-3">
                                    <NewSelect
                                        ...
                                        onChange={(valueOption) => {
                                            ...
                                            getCreditSubGeneralLedgerDDL(`/fino/FinanceCommonDDL/GetBusinessTransactionDDL?accountId=${profileData?.accountId
                                                }&businessUnitId=${selectedBusinessUnit?.value
                                                }&generalLedgerId=${valueOption?.value
                                                }`, (data) => {
                                                    const newData = data?.map((item) => ({
                                ...
                                <div className="col-lg-3">
                                    <button
                                        ...
                                        disabled={!values?.sbu || !values?.costCenter || !values?.costElement || !values?.profitCenter ||
                                            !values?.debitGeneralLedger || !values?.debitSubGeneralLedger || !values?.creditGeneralLedger ||
                                            !values?.creditSubGeneralLedger || !values?.salaryComponent || !values?.strType
                                        }
                                    >
                                        Add
                                    </button>
