...
export default function DamDeliveryBill() {
  ...
  const getData = (values, searchTerm) => {
    getGridData(
      `/tms/LigterLoadUnload/GetLighterDumpToTruckDeliveryPagination?BusinessUnitId=${buId}&lighterVesselId=${
        values?.lighterVessel?.value
      }&shipPointId=${values?.shipPoint?.value}&FromDate=${
        values?.fromDate
      }&ToDate=${values?.toDate}&isDumpToTruckApprove=${0}`,
      (resData) => {
