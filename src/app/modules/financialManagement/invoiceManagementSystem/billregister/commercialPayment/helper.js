  ...
  try {
    ...
    if (!filterData?.every(item=>item?.businessPartnerName === filterData[0]?.businessPartnerName)) {
      ...
      const res = await axios.post(
        `/imp/FormulaForCalculation/CommercialCostingForTypeTwo?accountId=${accId}&businessUnitId=${buId}&plantId=${plantId}&sbuId=${sbuId}&transactionDate=${_dateFormatter(new Date())}&costId=${0}&businessPartnerId=${supplierId}&totalAmount=${0}&typeId=${2}&actionById=${actionById}&SupplierBillRef=${supplierBillRef}`,
        dataSet
      );
      setDisabled(false);
      toast.success(
...
export const getCommercialCostingServiceBreakdown = async(referenceId, setter) => {
  try{
    const res= await axios.get(`/imp/AllCharge/GetCommercialCostingServiceBreakdown?referenceId=${referenceId}`)
    if(res.status === 200){
      setter(res?.data)
    }
  }catch(err){
...
export const createCommercialBreakdownForBill= async(payload, setIsLoading)=>{
  try{
    setIsLoading(true)
    const res = await axios.post("/imp/AllCharge/CreateCommercialBreakdownForBill", payload)
