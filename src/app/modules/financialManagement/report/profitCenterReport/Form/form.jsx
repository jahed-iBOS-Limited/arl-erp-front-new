...
export default function FormCmp({ initData }) {
  ...
  const [controllingUnitDDL, getControllingUnitDDL, controllingUnitLoader] =
    useAxiosGet();
  const [
    profitCenterGroupDDL,
    getProfitCenterGroupDDL,
    profitCenterGroupLoader,
  ] = useAxiosGet();
  const [profitCenterDDL, getProfitCenterDDL, profitCenterLoader] =
    useAxiosGet();

  useEffect(() => {
    getControllingUnitDDL(
      `/costmgmt/ControllingUnit/GetControllingUnitDDL?AccountId=${accountId}&BusinessUnitId=${selectedBusinessUnit?.value}`
    );
            ...
            <Form className="form form-label-right">
              <div className="form-group row global-form">
                <div className="col-lg-3">
                  <NewSelect
                    ...
                    options={
                      [{ value: 0, label: 'All' }, ...controllingUnitDDL] || []
                    }
                    ...
                    onChange={(valueOption) => {
                      ...
                      if (valueOption) {
                        getProfitCenterGroupDDL(
                          `/costmgmt/ProfitCenterGorup/GetProfitCenterGroupDDL?AccountId=${accountId}&BusinessUnitId=${selectedBusinessUnit?.value}&ControllunitId=${valueOption?.value}`
                        );
                      }
                    }}
                ...
                <div className="col-lg-3">
                  <NewSelect
                    ...
                    options={
                      [{ value: 0, label: 'All' }, ...profitCenterGroupDDL] ||
                      []
                    }
                    ...
                    onChange={(valueOption) => {
                      ...
                      if (valueOption) {
                        getProfitCenterDDL(
                          `/costmgmt/ProfitCenter/GetProfitcenterDDL?AccountId=${accountId}&BusinessUnitId=${selectedBusinessUnit?.value}&ControllingUnit=${values?.controllingUnit?.value}&GroupId=${valueOption?.value}`
                        );
                      }
                    }}
                ...
                <div style={{ marginTop: '18px' }} className="col-lg-3">
                  ...
                  <button
                    ...
                    onClick={() => {
                      setRowDto([]);
                      getProfitCenterReconcileReport({
                        getReconcileDto,
              ...
              <div>
                {reconcileDto?.length ? (
                  ...
                  <Table rowDto={rowDto} landingValues={values} />
                )}
              </div>
            </Form>
          </>
