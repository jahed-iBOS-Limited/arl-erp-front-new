export const approveHandeler = ({
  ...
  const isBankRelatedAction = ['Bank Payments', 'Cash Transfer'].includes(
    actionName
  );
  ...
  const commonBankFields = {
    ...
    intRequestGLId:
      item?.intRequestGLId?.generalLedgerId || item?.intRequestGlid || 0,
    strRequestGlName:
      item?.strRequestGlName?.generalLedgerName || item?.strRequestGlName || '',
    strRequestGlCode:
      item?.strRequestGlCode?.generalLedgerCode || item?.strRequestGlCode || '',
    intGivenBankId: isBankRelatedAction
      ? bankPaymentValues?.bankAcc?.bankId
      : item.intGivenBankId || 0,
    strGivenBankName: isBankRelatedAction
      ? bankPaymentValues?.bankAcc?.bankName
      : item.strGivenBankName || '',
    intGivenBankBranchId: isBankRelatedAction
      ? bankPaymentValues?.bankAcc?.bankBranch_Id
      : item.intGivenBankBranchId || 0,
    strGivenBankBranchName: isBankRelatedAction
      ? bankPaymentValues?.bankAcc?.bankBranchName
      : item.strGivenBankBranchName || '',
    strGivenBankAccountNumber: isBankRelatedAction
      ? bankPaymentValues?.bankAcc?.bankAccNo
      : item.strGivenBankAccountNumber || '',
    strGivenBankAccountName: isBankRelatedAction
      ? bankPaymentValues?.bankAcc?.label
      : item.strGivenBankAccountName || '',
    strGivenBankAccountId: isBankRelatedAction
      ? bankPaymentValues?.bankAcc?.value
      : item?.strGivenBankAccountId || 0,
    strGivenBankRouting: isBankRelatedAction
      ? bankPaymentValues?.bankAcc?.bankRouting
      : item?.strGivenBankRouting || '',
    intGivenGlid: isBankRelatedAction
      ? bankPaymentValues?.bankAcc?.generalLedgerId
      : item?.intGivenGlid || 0,
    strGivenGlCode: isBankRelatedAction
      ? bankPaymentValues?.bankAcc?.generalLedgerCode
      : item?.strGivenGlCode || '',
    strGivenGlName: isBankRelatedAction
      ? bankPaymentValues?.bankAcc?.generalLedgerName
      : item?.strGivenGlName || '',
  };
  ...
  const finalPayload = ['Bank Receipt'].includes(actionName)
    ? forFundRecievedPayload
    : commonPayload;
