...
export default function FundTransferApproval({ viewType }) {
  ...
  const getLandingData = (values, pageNo, pageSize, searchValue = '') => {
    ...
    const isTransferCreated =
      values?.status?.value === null
        ? ''
        : `&isTransferCreated=${values?.status?.value}`;
    getGridData(
                      ...
                      <tbody>
                        {gridData?.data?.map((item, index) => (
                          <tr key={index}>
                            ...
                            <td
                              className={`bold text-center ${item?.isTransferCreated === 1 ? 'text-success' : 'text-warning'}`}
                            >
                              {item?.isTransferCreated === 1
                                ? 'Fund Transfred'
                                ...
                                {item?.isTransferCreated !== 1 && (
                                  <span
                                    onClick={() => {
                                      ...
                                      const isBankToBank =
                                        item?.strTransferBy === 'Bank To Bank';
                                        ...
                                        if (isContra) {
                                          return {
                                            ...
                                            bankAcc: {
                                              ...
                                              bankBranch_Id:
                                                item?.intGivenBankBranchId,
                                              bankBranchName:
                                                item?.strGivenBankBranchName,
                                              value:
                                                item?.strGivenBankAccountId,
                                              label:
                                                item?.strGivenBankAccountName,
                                              bankAccNo:
                                            ...
                                            sendToGLBank: {
                                              value:
                                                item?.intRequestedBankAccountId,
                                              label:
                                                item?.strRequestedBankAccountName,
                                              bankAccNo:
                                                item?.strRequestedBankAccountNumber,
                                              generalLedgerCode:
                                                item?.strRequestGlCode,
                                            },
                                            paidTo:
                                              item?.strRequestedBankAccountName ||
                                              '',
                                          ...
                                          return {
                                            ...
                                            intRequestToUnitId:
                                              item?.intRequestToUnitId,
                                            transaction: {
                                              value: item?.strRequestPartnerId,
                                              label:
                                                item?.strRequestPartnerName,
                                              code:
                                                item?.strRequestPartnerCode ||
                                                '',
                                            },
                                            partnerBankAccount: {
                                              value:
                                              ...
                                              bankBranchId:
                                                item?.intRequestedBankBranchId,
                                              bankAccountNo:
                                                item?.strRequestedBankAccountNumber,
                                              bankName:
                                                item?.strRequestedBankName,
                                              routingNo:
                                                item?.strRequestedBankRouting ||
                                                '',
                                            },
                                            paidTo:
                                              item?.strRequestPartnerName || '',
                                      ...
                                      const isTransferNotCreated =
                                        item?.isTransferCreated === 0;

                                      // Redirect based on transfer type
                                      if (
                                        isBankToBank &&
                                        isApproved &&
                                        isTransferNotCreated
                                      ) {
                                        history.push({
                                          ...
                                          selectedFormValues: {
                                            bankAcc: {
                                              ...
                                              bankBranch_Id:
                                                item?.intRequestedBankBranchId,
                                              bankBranchName:
                                                item?.strRequestedBankBranchName,
                                              value:
                                                item?.intRequestedBankAccountId,
                                              label:
                                                item?.strRequestedBankAccountName,
                                            ...
                                            transaction: {
                                              ...
                                              code:
                                                item?.strGivenstrPartnerCode ||
                                                '',
                                            },
                                            receiveFrom:
                                              item?.strGivenPartnerName || '',
                                          },
                                        };
