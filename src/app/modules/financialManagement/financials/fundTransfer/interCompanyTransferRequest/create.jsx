...
const getSchema = () => {
  const validationSchema = Yup.object().shape({
    ...
    requestAmount: Yup.number()
      ...
      .min(1, 'Request Amount must be positive'),
...
export default function InterCompanyTransferRequestCreate() {
  ...
  const saveHandler = (values, cb) => {
    const payload = {
      ...
      strRequestPartnerCode:
        values?.requestToPartner?.strBusinessPartnerCode || '',
      ...
      strRequestedBankBranchName:
        values?.receivingAccount?.bankBranchName || '',
      ...
      strGivenstrPartnerCode:
        values?.sendingPartner?.strBusinessPartnerCode || '',
    };
    onCreateHandler(
  ...
  return (
    ...
    >
      {({
        ...
        <>
          {(saveLoader || sendingPartnerLoading || requestPartnerLoading) && (
            <Loading />
          )}
          <IForm
            title="Inter Company Transfer Request Create"
              ...
              <div className="form-group global-form row">
                    ...
                    onChange={(valueOption) => {
                      ...
                      if (valueOption) {
                        getSendingPartnerByOwnUnit(
                          `/partner/PManagementCommonDDL/GetBusinessPartnerByOwnUnit?businessUnitId=${selectedBusinessUnit?.value}&businessPartnerOwnUnitId=${valueOption?.value}`,
                          (res) => {
                            if (
                              res?.intBusinessPartnerId &&
                              res?.strBusinessPartnerName
                            ) {
                              setFieldValue('sendingPartner', {
                                ...res,
                                value: res?.intBusinessPartnerId,
                                label: res?.strBusinessPartnerName,
                              });
                            } else {
                              toast.warn(
                                'Please Business Partner Configure Properly'
                              );
                            }
                          }
                        );
                        getRequestPartnerByOwnUnit(
                          `/partner/PManagementCommonDDL/GetBusinessPartnerByOwnUnit?businessUnitId=${valueOption?.value}&businessPartnerOwnUnitId=${selectedBusinessUnit?.value}`,
                          (res) => {
                            if (
                              res?.intBusinessPartnerId &&
                              res?.strBusinessPartnerName
                ...
                <div className="col-lg-3">
                  <NewSelect
                    ...
                    onChange={(valueOption) =>
                      setFieldValue('requestToPartner', valueOption)
                    }
                    errors={errors}
                    touched={touched}
                    isDisabled={true}
                ...
                <div className="col-lg-3">
                  <NewSelect
                    ...
                    onChange={(valueOption) =>
                      setFieldValue('receivingAccount', valueOption)
                    }
                    errors={errors}
                    touched={touched}
                  />
                ...
                <div className="col-lg-3">
                  <NewSelect
                    ...
                    onChange={(valueOption) =>
                      setFieldValue('sendingPartner', valueOption)
                    }
                    errors={errors}
                    touched={touched}
                    isDisabled={true}
                ...
                <div className="col-lg-3">
                  <InputField
                    ...
                    type="date"
                    onChange={(e) =>
                      setFieldValue('expectedDate', e.target.value)
                    }
                    min={_todayDate()}
                  />
                </div>
                ...
                <div className="col-lg-3">
                  <InputField
                    ...
                    type="number"
                    onChange={(e) =>
                      setFieldValue('requestAmount', e.target.value)
                    }
                  />
                </div>
