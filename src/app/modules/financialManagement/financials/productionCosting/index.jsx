...
export default function ProductionCost() {
  ...
  const [
    cheakingMonthClosingState,
    getCheakingMonthClosingState,
    closingLoader,
  ] = useAxiosGet();
      ...
      yesAlertFunc: async () => {
        const payload = {
          updateCostLsit: rowData?.map((item) => ({
            ...
            updateProductCost:
              item?.ActualAvgRate * item?.numTransactionQuantity,
          })),
        };
          ...
          >
            <Form>
              <div>
                  ...
                  <div className="col-lg-3">
                    ...
                    <SearchAsyncSelect
                      ...
                      loadOptions={(v, resolve) => {
                        ...
                        return axios
                          .get(
                            `/mes/MesDDL/GetUnitWiseFGItemDDL?AccountId=${profileData?.accountId}&BusinessUnitId=${selectedBusinessUnit?.value}&Search=${v}`
                          )
                          .then((res) => {
                  ...
                  <div className="col-lg-2">
                    <InputField
                      ...
                      onChange={(e) => {
                        ...
                        getCheakingMonthClosingState(
                          `/wms/InventoryTransaction/CheckMonthClosing?BusinessUnitId=${selectedBusinessUnit?.value}&dteDate=${e.target.value}`
                        );
                      }}
                    />
                  </div>
                  ...
                  <div className="col-lg-2">
                    <button
                      ...
                      onClick={() => {
                        ...
                        getRowData(
                          `/fino/Report/GetProductionOrderActualRate?intBusinessUnitId=${selectedBusinessUnit?.value}&dteFromDate=${values?.fromDate}&dteToDate=${values?.toDate}&intItemId=${values?.item?.value}`
                        );
                      }}
                      disabled={
                        !values?.item || !values?.fromDate || !values?.toDate
                      }
                    >
                      View
                    </button>
                ...
                {productionCostPermission?.isView ? (
                    ...
                    <tbody>
                      {rowData?.map((item, index) => (
                        ...
                        <tr key={index}>
                          ...
                          <td className="text-center">
                            {item?.strReferenceCode}
                          </td>
                          <td className="text-right">
                            {item?.numTransactionQuantity}
                          </td>
                          <td className="text-right">
                            {_formatMoney(item?.RuningAvgRate)}
                          </td>
                          <td className="text-right">
                            {_formatMoney(item?.ActualAvgRate)}
                          </td>
                          <td className="text-right">
                            {_formatMoney(
                              item?.ActualAvgRate * item?.numTransactionQuantity
