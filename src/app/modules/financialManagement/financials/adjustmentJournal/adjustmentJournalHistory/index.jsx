...
export default function HistoryModal({ adjustmentJournalId, landingItem }) {
  const [
    ...
    singleJournalDataLoader,
    setSingleJournalData,
  ] = useAxiosGet();
  const [journalHistoryData, getJournalHistoryData, journalHistoryDataLoader] =
    useAxiosGet();
  ...
  useEffect(() => {
    if (adjustmentJournalId) {
      getSingleJournalData(
        `/fino/AdjustmentJournal/GetAdjustmentJournalByIdForReport?adjustmentJournalId=${adjustmentJournalId}&accountingJournalTypeId=7&BusinessUnitId=${selectedBusinessUnit?.value}`,
        (data) => {
          console.log('data', data);
          setSingleJournalData(data?.[0]);
          getJournalHistoryData(
            `/fino/Accounting/GetAccountingJournalLOGHistory?accountingJournalId=${adjustmentJournalId}&accountingJournalTypeId=${landingItem?.intAccountingJournalTypeId || 0}`
          );
        }
      );
    }
              ...
              <div>
                <div className="d-flex flex-column justify-content-center align-items-center mt-2">
                  <div className="d-flex flex-column justify-content-center align-items-center mt-2">
                    ...
                    <span>
                      {singleJournalData?.objHeader?.businessUnitAddress}
                    </span>
