...
export default function HistoryModal({ journalId, journalTypeId }) {
  const [singleJournalData, getSingleJournalData, singleJournalDataLoader] =
    useAxiosGet();
  ...
  useEffect(() => {
    if (journalId) {
      getSingleJournalData(
        `/fino/CommonFino/GetBankJournalReport?JournalId=${journalId}&AccountingJournalTypeId=${journalTypeId}&BusinessUnitId=${selectedBusinessUnit?.value}`,
        (data) => {
          // setSingleJournalData(data?.[0]);
          getJournalHistoryData(
            `/fino/Accounting/GetAccountingJournalLOGHistory?accountingJournalId=${journalId}&accountingJournalTypeId=${journalTypeId}`
          );
        }
      );
    }
