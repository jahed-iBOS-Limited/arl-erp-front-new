...
const TableData = ({ adviceReportData, setAdviceReportData }) => {
  const totalAmount = useMemo(() => adviceReportData.reduce((acc, item) => acc + +item.numAmount, 0), [adviceReportData]);
      ...
      <table className={"table table-striped table-bordered global-table mt-0 table-font-size-sm advice_table"}>
        ...
        <tbody>
          {adviceReportData?.map((item, index) => (
            <tr key={index} className={(item?.printCount || item?.mailCount) ? "font_color_red":""}>
              ...
              <td>P-{item?.printCount}, M-{item?.mailCount} </td>
            </tr>
          ))}
          {adviceReportData.length > 0 && (
            <tr>
