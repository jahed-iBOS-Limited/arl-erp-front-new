...
export const employeeBasicInformation_landing_api_new = async (
  ...
  try {
    const res = await axios.get(
      `/hcm/EmployeeBasicInformation/EmployeeBasicInfoLandingPasignationForReRegistration?searcTerm=${searchPath}&Accountid=${accId}&BusinessUnitId=${buId}${strFromDate}${strToDate}&viewOrder=desc&PageNo=${pageNo}&PageSize=${pageSize}&isDownload=${isDownload}`
    )
