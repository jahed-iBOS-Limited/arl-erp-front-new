...
// create extend
export const saveEmpGroupExtend = async (createData, cb, setDisabled) => {
  try {
    setDisabled(true)
...
export const getEmployeeGroupNameLanding = async (accountId, setter) => {
  try {
    const res = await axios.get(
      `hcm/EmployeeConfigure/GetEmployeeGroupNameLanding?AccountId=${accountId}&PageNo=1&PageSize=100`
    );
    if (res.status === 200 && res?.data) {
...
// create group name
export const createEmployeeGroupNew = async (createData, cb, setDisabled) => {
  try {
    setDisabled(true)
    const res = await axios.post(
