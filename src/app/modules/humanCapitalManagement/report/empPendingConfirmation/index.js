...
const EmpPendingConfirmationReport = () => {
    ...
    <>
      ...
      >
        {({ values, errors, touched, setFieldValue }) => (
          <>
            <ICard printTitle="Print" title="Employee Confirmation List" isPrint={true} isShowPrintBtn={true} componentRef={printRef}>
                        ...
                        <div className="row">
                          ...
                          <div style={{ marginTop: "15px" }} className="col-lg-4 d-flex">
                            <button
                              ...
                              onClick={() => {
                                getEmployeeJoiningConfirmationReport(1, _firstDateofMonth(), values?.endDate, setRowDto, setLoading);
                              }}
                      ...
                      {rowDto?.length > 0 &&
                        rowDto?.map((item, index) => (
                          <tr key={index}>
                            ...
                            <td className="text-right">{_formatMoney(item?.numGrossSalary)}</td>
                            <td>{item?.strSupervisorName}</td>
                            <td>{item?.strEmail}</td>
                            <td>{item?.OfficialContact === "NULL" || item?.OfficialContact === "0" ? "" : item?.OfficialContact}</td>
                          </tr>
                        ))}
                    </tbody>
                  </table>
...
export default EmpPendingConfirmationReport;
