...
export const getEmpInfoById = async (valueOption, setFieldValue) => {
  try {
    let res = await Axios.get(`/hcm/HCMDDL/GetEmployeeDetailsByEmpId?EmpId=${valueOption?.value}`);
    let {hrPosition, employeeInfoDesignation, employeementType, workplaceGroup, employeeBusinessUnit, employeeInfoDepartment, supervisor} = res?.data
    setFieldValue(
      "employeeInfo",
      `${employeeInfoDesignation},${employeeInfoDepartment},${employeeBusinessUnit}`
    );
    setFieldValue("employee", {...valueOption, hrPosition, employeeInfoDesignation, employeementType, workplaceGroup, employeeBusinessUnit, employeeInfoDepartment, supervisor})
  } catch (error) {
    return null;
  }
};
