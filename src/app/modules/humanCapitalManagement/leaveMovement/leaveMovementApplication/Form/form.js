...
const validationSchema = Yup.object().shape({
  ...
  toDate: Yup.string().required("To date is required").typeError("To date is required"),
  reason: Yup.string()
    .min(2, "Minimum 2 symbols")
    .required("Reason is required"),
  address: Yup.string()
...
export default function _Form({
  ...
  useEffect(() => {
    if (location && profileData?.accountId) {
      if (initData?.leaveType === 1) {
        getLeaveTypeDDL(1, location?.state?.employee?.value, setLeaveTypeDDL);
      } else {
        getLeaveTypeDDL(0, location?.state?.employee?.value, setLeaveTypeDDL);
      }
    }
  }, [location, profileData]);

  useEffect(() => {
    if(location?.state?.privacyType === "1" && selectedLeaveType === 1){
      let data = [...leaveTypeDDL];
     const modifyData =  data.filter((item) => item.value !== 7 && item?.value !== 9)
      setUpdateLeaveTypeDDL([...modifyData]);
    }
   

  },[leaveTypeDDL, selectedLeaveType])
  ...
  const plLeaveFromDateHandler = (setFieldValue, value) => {
    ...
    if (plLeaveBalance?.[0]?.remainingDays > 0) {
      let toDate = addDaysToADate(value, plLeaveBalance?.[0]?.remainingDays - 1);
                ...
                <div className="col-lg-6">
                  <div className="row bank-journal bank-journal-custom bj-left mr-1">
                        ...
                        <>
                          ...
                          <SearchAsyncSelect
                            ...
                            handleChange={(valueOption) => {
                              ...
                              getEmpInfoById(valueOption, setFieldValue, "employeeName")
                            }}
                    ...
                    <div className="col-lg-4 mb-2">
                      <NewSelect
                        name="reasonType"
                        options={location?.state?.privacyType === "1" && values?.leaveType === 1 ? updateLeaveTypeDDL : leaveTypeDDL || []}
