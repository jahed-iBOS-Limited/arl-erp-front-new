...
export const employeeContactTypeDDL_api = async (setter) => {
  try {
    const res = await Axios.get("/hcm/HCMDDL/EmployeeContactTypeDDL");
...
export const createEmpOthersContactInfo_api = async (
  payload,
  cb,
  setDisabled
) => {
  try {
    ...
    const res = await Axios.post(
      "/hcm/EmpOthersContactInfo/CreateEmpOthersContactInfo",
      payload
    );
    if (res.status === 200) {
...
//GetEmpOthersContactInfoById
export const getEmpOthersContactInfoById_api = async (id, setter) => {
  try {
    const res = await Axios.get(
      `/hcm/EmpOthersContactInfo/GetEmpOthersContactInfoById?EmployeeId=${id}`
    );
    if (res.status === 200 && res.data) {
      const data = res.data;
...
export const editEmpOthersContactInfo_api = async (
  payload,
  cb,
  setDisabled
) => {
  ...
  try {
    const res = await Axios.put(
      "/hcm/EmpOthersContactInfo/EditEmpOthersContactInfo",
      payload
    );
    if (res.status === 200) {
      toast.success(res?.data?.message || "Submitted Successfully");
