...
const PrintPage = ({ tenderDetails: { header, rows } }) => {
  ...
  const firstHalfDataOnTable = rows?.length > 0 && rows?.slice(0, Math.floor(rows?.length / 2))
  const lastHalfDataOnTable = rows?.length > 0 && rows?.length !== 1 && rows?.slice(Math.floor(rows?.length / 2))
          ...
          <tbody>
            <tr style={{ width: "100%" }}>
              <td rowSpan={2} colspan={2} style={{ width: "30%", textAlign: "left" }}>
                <strong style={{ display: 'block' }}>PART – A</strong> <strong style={{ display: 'block' }}>(FOREIGN PART)</strong> TRANSPORTATION OF {header?.foreignQty} {header?.uomname} (+/-10%) {" "}
              ...
              <td colSpan={4}>
                ...
                (+/-10%) {" "}{header?.itemName} FROM MOTHER VESSEL AT OUTER ANCHORAGE
            ...
            {firstHalfDataOnTable?.map((item, index) => {
              return (
                <>
                  <tr>
                    {index === 0 && <>
                      <td rowSpan={firstHalfDataOnTable.length} colSpan={2}></td>

                    </>
                    }
                    <>
                      <td style={{ textAlign: 'left' }} rowSpan={!firstDataOnTable ? 2 : 1}>{item?.godownName}</td>
                      <td style={{ textAlign: 'right', fontWeight: "bold", width: "100px" }} rowSpan={!firstDataOnTable ? 2 : 1}>{item?.quantity}</td>
                      <td style={{ textAlign: 'right', fontWeight: "bold", width: "100px" }} rowSpan={!firstDataOnTable ? 2 : 1}>{item?.perQtyTonPriceBd}</td>
                      <td style={{ textAlign: 'center' }} rowSpan={!firstDataOnTable ? 2 : 1}>{item?.perQtyPriceWords}</td>
                    </>
                  </tr>
            ...
            {lastHalfDataOnTable && lastHalfDataOnTable?.map((item, index) => {
              return (
                <>
                  <tr>
                    {index === 0 && <>
                      <td rowSpan={lastHalfDataOnTable.length} colSpan={2}></td>
                    </>
                    }
                    <>
            ...
            <tr height={500}>
              ...
              <td colSpan={3} style={{
                ...
                textAlign: 'center'
              }}>Name and Address of the PROPRIETOR</td>
            </tr>
          </tbody>
        </table>
      }
