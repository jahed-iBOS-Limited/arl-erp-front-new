...
const DayByDayUnloadAndDelivery = () => {
  ...
  const getReportId = (values) => {
    const reportId = [1].includes(values?.reportType?.value)
      ? `00aa2f41-2520-40ad-9644-fe75486f5a20`
      : [2].includes(values?.reportType?.value)
        ? `03f654d8-b8e5-45c7-80c4-959d731953ec`
        : '';
  ...
  const parameterValues = (values, buId) => {
    ...
    return [1].includes(values?.reportType?.value)
      ? [
          ...
          {
            name: 'intMotherVesselId',
            value: `${+values?.motherVessel?.value}`,
          },
