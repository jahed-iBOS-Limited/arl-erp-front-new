...
export const createDamageCategory = async (payload, setIsLoading, cb) => {
  ...
  try {
    let res = await axios.post(
      `/rtm/DamageCategory/CreateDamageCategory`,
      payload
    );
    if (res?.status === 200) {
      cb();
...
export const editDamageType = async (payload, setIsLoading) => {
  ...
  try {
    let res = await axios.put(
      `/rtm/DamageCategory/EditDamageCategory`,
      payload
    );
    if (res?.status === 200) {
...
export const getDamageCategoryById = async (id, setter) => {
  try {
    let res = await axios.get(
...
export const getDamageTypeDDL = async (accId, buId, setter) => {
  try {
    let res = await axios.get(
      `/rtm/RTMDDL/DamageTypeDDL?AccountId=${accId}&BusinessUnitId=${buId}`
    );
    if (res?.status === 200) {
      setter(res?.data);
    }
