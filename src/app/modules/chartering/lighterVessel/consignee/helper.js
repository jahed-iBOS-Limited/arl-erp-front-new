...
export const getConsigneeList = async (
  ...
  try {
    ...
    // const res = await axios.get(
    //   `https://imarine.ibos.io/domain/LighterConsignee/LighterConsigneePagination?AccountId=${accId}&BusinessUnitId=${buId}&viewOrder=desc&PageNo=${pageNo}&PageSize=${pageSize}`
    // );
    setter(res?.data);
    setLoading(false);
  } catch (error) {
...
export const createConsignee = async (data, setLoading, cb) => {
  ...
  try {
    const res = await axios.post(
      `https://imarine.ibos.io/domain/LighterConsignee/CreateLighterConsignee`,
      data
    );
...
export const saveEditedConsignee = async (data, setLoading, cb) => {
  ...
  try {
    const res = await axios.put(
      `https://imarine.ibos.io/domain/LighterConsignee/EditLighterConsignee`,
      data
    );
    toast.success(res?.data?.message);
    cb();
