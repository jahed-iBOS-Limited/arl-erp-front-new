...
export const getBunkerCostLandingData = async (
  ...
  try {
    const res = await axios.get(
      `https://imarine.ibos.io/domain/BunkerCost/GetBunkerCostLanding?AccountId=${accId}&BusinessUnitId=${buId}&VesselId=${vesselId}&VoyageId=${voyageId}&viewOrder=desc&PageNo=${pageNo}&PageSize=${pageSize}`
    );
    setter(res?.data);
    setLoading(false);
  } catch (error) {
