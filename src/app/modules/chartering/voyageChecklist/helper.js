...
export const getVoyageByIdShow = async (
  voyageId,
  setter,
  setLoading,
) => {
  setLoading && setLoading(true);
  try {
    const res = await axios.get(`/hcm/VoyageChecklist/GetVoyageById?VoyageId=${voyageId}`);
    if(res?.data){
      setter(res?.data);
    }
    setLoading && setLoading(false);
...
export const getVoyageChecklistPasignation = async (
  ...
  setLoading && setLoading(true);
  try {
    const res = await axios.get(`/hcm/VoyageChecklist/VoyageChecklistPasignation?viewOrder=${viewOrder}&pageNo=${pageNo}&pageSize=${pageSize}`);
    if(res?.data){
      setter(res?.data);
    }
    setLoading && setLoading(false);
