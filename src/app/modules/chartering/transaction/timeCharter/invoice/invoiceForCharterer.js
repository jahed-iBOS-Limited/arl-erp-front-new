...
export default function InvoiceForCharterer({
  invoiceHireData,
  formikprops,
  rowData,
  rowDtoHandler,
  ...
  const OHD = offHireDuration ? offHireDuration : 0;
              ...
              onChange={(e) => {
                ...
                const newArr = copy?.map((item) => {
                  if (item?.key === "hdto") {
                    return {
                      ...item,
                      duration: diffDate - OHD,
                      // duration: diffDate,
                      credit: (diffDate - OHD) * invoiceHireData?.dailyHire,
                    };
                  }
                  if (item?.key === "hac") {
                    return {
                  ...
                  if (item?.key === "cve") {
                    return {
                      ...
                      duration: diffDate - OHD,
                      // duration: diffDate,
                      credit: ((diffDate - OHD) * invoiceHireData?.cveday) / 30,
                    };
                  }
                  if (item?.key === "hbc") {
                    return {
    ...
    <table className="table mt-3 bj-table bj-table-landing">
        ...
        <tbody>
          {rowData?.map((item, index) => {
            if (item?.isChecked) {
              totalCredit = totalCredit + +item?.credit || 0;
            }
            if (item?.isChecked) {
              totalDebit = totalDebit + +item?.debit || 0;
            }
            // console.log("credit", item?.credit, item?.description);
            // console.log("debit", item?.debit, item?.description);
            return (
              <tr key={index}>
                {/* SL */}
                <td
                ...
                {/* Debit will be Debit For Charterer */}
                <td
                  className={`${
                    item?.isChecked ? "isCheckedTrue" : "isCheckedFalse"
                  } text-right`}
                ...
                >
                  {item?.isDebit ? (
                    ...
                    _formatMoneyWithDoller(item?.debit?.toFixed(2)?.toString())
                  )}
                </td>

                {/* Credit Will be Credit For Charterer */}
                <td
                  className={`${
                    item?.isChecked ? "isCheckedTrue" : "isCheckedFalse"
                  } text-right`}
                ...
                >
                  {item?.isCredit ? (
                    ...
                    _formatMoneyWithDoller(item?.credit?.toFixed(2)?.toString())
                  )}
                </td>
