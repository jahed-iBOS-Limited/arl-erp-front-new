...
export default function ConfigItemPlantWareHouse({isViewPage,onSuccess}) {
  ...
  const defaultSetter = (payload) => {
    ...
    // ) 
    const isExists = defaultRowDto.find((itm) => (itm.warehouseId === payload.warehouseId && itm?.plantId === payload?.plantId))

    if(isExists){
      return toast.error("Warehouse & Plant already exists !", { toastId: shortid() });
    }
  ...
  return (
    <Card>
      <CardHeader  title={isViewPage ?"Config Item Plant Warehouse" : "Edit Config Item Plant Warehouse"} >
        <CardHeaderToolbar>
          {
            !isViewPage && (
              <>
