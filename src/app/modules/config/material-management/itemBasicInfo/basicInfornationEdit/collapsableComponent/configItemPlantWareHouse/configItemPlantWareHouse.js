...
export default function ConfigItemPlantWareHouse({ isViewPage, onSuccess }) {
  ...
  const getDataById = async (id) => {
    ...
    if (status === 200) {
      const meta = data[0];
      meta?.uomData && setExistUOMData(meta?.uomData || {});
      meta?.uomData &&
        setRowDto([
          {
            baseUomId: meta?.uomData?.baseUom,
            baseUomName: meta?.uomData?.baseUomName,
            convertedUom: meta?.uomData?.convertedUom,
            convertedUomName: meta?.uomData?.convertedUomName,
            numConversionRate: meta?.uomData?.conversionRate,
          },
        ]);
    }
  };
