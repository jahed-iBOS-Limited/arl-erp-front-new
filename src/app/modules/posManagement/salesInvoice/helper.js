...
export const saveDeliveryPos= async (payload, setHeader, setRow, setDisabled, cb) => {
  setDisabled(true);
  try {
    const res = await Axios.post("/partner/Pos/POSCreateDeliverySpr", payload);
    if(res.status === 200){
...
export const saveHoldingDeliveryPos= async (payload, setDisabled, cb) => {
  setDisabled(true);
  try {
    const res = await Axios.post("/partner/Pos/CreatePOSHoldInformation", payload);
    toast.success(res?.data?.message);
    setDisabled(false);
    cb()
  } catch (error) {
...
export const getSalesInvoiceById = async (
  ...
  try {
    ...
    console.log(header)
    const customerData= await Axios.get(`/partner/Pos/GetCustomerNameDDL?SearchTerm=${header?.partnerContactNo}&AccountId=${accountId}&BusinessUnitId=${bussinessUnitId}&WarehouseId=${header?.warehouseId}`)
    const headerResponse={
      customer:{
...
export const getSalesItemById = async (accId, buId, whId, salesItemId, setter, values) => {
  try {
    const res = await Axios.get(
...
export const salesDeliveryReturn= async (payload, setHeader, setRow, setDisabled, cb) => {
  setDisabled(true);
  try {
...
export const customerCreditRecovery= async (payload) => {
  try{
    const res=await Axios.post("/partner/Pos/SetCustomerCreditRecovery", payload)
    if(res?.status=== 200){
      toast.success(res?.data?.message);
    }
  }catch (error) {
...
const getItem=(itemId)=> {
  return Axios.get(`/item/ItemSales/GetItemSalesDetailsforPOSDDL?itemId=${itemId}`);
}
...
export const getSalesOrderByOrderId = async (
  ...
  try {
    ...
    const customerData= await Axios.get(`/partner/Pos/GetCustomerNameDDL?SearchTerm=${header?.customerName}&AccountId=${accId}&BusinessUnitId=${buId}&WarehouseId=${header?.warehouseId}`)
    ...
    const responseRow= await Promise.all(row.map(async(itm)=>{
...
export const deleteHoldingInvoice= async (deliveryId)=>{
  try{
    const res= await Axios.put(`/partner/Pos/DeleteHoldInformation?deliveryId=${deliveryId}`)
    if(res.status===200){
      toast.success(res.data?.message)
    }
  }catch(error){
