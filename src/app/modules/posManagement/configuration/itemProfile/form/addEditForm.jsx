...
import { getWareHouseDDL, updateItemProfile } from '../../helper';
...
const ItemProfileForm = () => {
  ...
  const saveHandler = async (values) => {
    const updatedRowDto = rowDto.filter(
      (data) => data.isUpdated === true && data.mrp > 0
    );
  ...
  return (
    <>
      <IForm title={'Update Sales Rate'} getProps={setObjprops}>
        <Form
          {...objProps}
          initData={initData}
