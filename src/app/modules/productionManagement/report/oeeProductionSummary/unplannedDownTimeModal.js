...
export default function UnPlannedDownTimeModal({ values }) {
  const [rowData, getRowData, loadingRowData] = useAxiosGet();

  useEffect(() => {
    getRowData(
      `/asset/AssetMaintanance/GetUnPlanDownTimeListByDate?FromDate=${values?.fromDate}&ToDate=${values?.toDate}&BusinessUnitId=${values?.businessUnit?.value}&PlantId=${values?.plant?.value}&ShopfloorId=${values?.shopFloor?.value}`
    );
