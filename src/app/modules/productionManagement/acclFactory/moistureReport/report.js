...
export default function MoistureReport() {
    ...
    const [reportData, getReportData, reportDataLoader, setReportData] = useAxiosGet();
    return (
        <Formik
                            ...
                            <div>
                                <div className="form-group row global-form">
                                    ...
                                    <div className="col-lg-2">
                                        <button
                                            ...
                                            onClick={() => {
                                                getReportData(`/hcm/QCTest/MoistureQcTransactionReport?BusinessUnitId=${selectedBusinessUnit?.value}&FromDate=${values?.fromDate}&ToDate=${values?.toDate}`)
                                            }}
                                        >
                                            Show
                                        </button>
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-lg-12">
                                        <table id="moisture-report-table" className="table table-striped table-bordered mt-3 bj-table bj-table-landing">
                                            <thead>
                                                <tr>
                                                    <th rowspan={3}>Sl</th>
                                                ...
                                                <tr>
                                                    {reportData?.data?.length > 0 &&
                                                        reportData?.data[0]?.rawMaterials?.map((item, index) => {
                                                            return (
                                                                <th>{item?.itemName}</th>
                                                            )
                                                        })
                                                    ...
                                                    {reportData?.data?.length > 0 &&
                                                        reportData?.data[0]?.finishedProducts?.map((item, index) => {
                                                            return (
                                                                <th>{item?.machineName} (
                                                                    {item?.itemName}
                                                                    )</th>
