...
export default function MoustureCreateEdit() {
    ...
    const saveHandler = (values, cb) => {
        ...
        saveMoistureData(id ? `/hcm/QCTest/MoistureQcTransactionEdit` : `/hcm/QCTest/CreateMoistureTest`, payload, cb, true)
    };
    useEffect(() => {
        if (selectedBusinessUnit) {
            getMachineDDL(`/hcm/QCTest/GetTransactionMachineNameDDL?BusinessUnitId=${selectedBusinessUnit?.value}`)
        }
        if (id) {
            getItemData(`/hcm/QCTest/GetMoistureQCItemConfigData?Date=${location?.state?.date}&Shift=${location?.state?.shiftName}&BusinessUnitId=${selectedBusinessUnit?.value}`,
                (data) => {
            ...
            setmodifyData(
                {
                    ...
                    shift: { value: location?.state?.shiftId, label: location?.state?.shiftName },
    ...
    return (
        ...
        >
            {({
                ...
                <>
                    {(moistureDataLoader || machineDDLloader || itemDataLoader) && <Loading />}
                    <IForm title={id ? "Edit Moisture Test" : "Create Moisture Test"} getProps={setObjprops}>
                        <Form>
                                    ...
                                    <NewSelect
                                        ...
                                        onChange={(valueOption) => {
                                            if (valueOption) {
                                                ...
                                                setFieldValue("remarks", "");
                                                getItemData(`/hcm/QCTest/GetMoistureQCItemConfigData?Date=${values?.dteDate}&Shift=${valueOption?.label
                                                    }&BusinessUnitId=${selectedBusinessUnit?.value}`,
                                                    (data) => {
                                                        const updatedData = data?.data?.map((itm) => {
                                                            return {
                                        ...
                                        <tbody>
                                            {
                                                tableData?.map((itm, idx) => (
                                                    <tr key={idx}>
                                                        ...
                                                        <td>{itm?.itemName}</td>
                                                        <td>
                                                            {itm?.itemTypeName === "Finished Products" ? (<NewSelect
                                                                options={machineDDL}
                                                                value={{
                                                                    value: itm?.machineId,
                                                                ...
                                                                onChange={(valueOption) => {
                                                                    if (valueOption) {
                                                                        ...
                                                                        data[idx].machineName = valueOption?.label;
                                                                        setTableData(data);
                                                                    } else {
                                                                        const data = [...tableData];
