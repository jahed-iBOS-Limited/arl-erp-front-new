...
export const onCreateOrEditCraneStopageDetails = ({
  ...
  const payload = rowList?.map((item) => ({
    craneStopageId: item?.id || 0,
    accountId: accountId,
    businessUnitId,
    entryDate: item?.date,
    shiftId: item?.shift?.value,
    shiftName: item?.shift?.label,
    craneId: item?.craneName?.value,
    craneName: item?.craneName?.label,
    breakdownTypeId: item?.breakdownType?.value,
    breakdownTypeName: item?.breakdownType?.label,
    duration: +item?.duration,
    stopesDetails: item?.stopageDetails,
    isActive: true,
