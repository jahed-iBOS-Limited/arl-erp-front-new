...
export const onCreateOrEditCraneStopageDetails = ({
  ...
  const payload = rowList?.map((item) => ({
    craneStopageId: item?.values?.id || 0,
    accountId: accountId,
    businessUnitId,
    entryDate: item?.values?.date,
    shiftId: item?.values?.shift?.value,
    shiftName: item?.values?.shift?.label,
    craneId: item?.values?.craneName?.value,
    craneName: item?.values?.craneName?.label,
    breakdownTypeId: item?.values?.breakdownType?.value,
    breakdownTypeName: item?.values?.breakdownType?.label,
    duration: +item?.values?.duration,
    stopesDetails: item?.values?.stopageDetails,
    isActive: true,
    createdAt: item?.values?.createdAt || _todayDate(),
    createdBy: item?.values?.createdBy || userId,
    updatedAt: item?.values?.id ? _todayDate() : "",
    updatedBy: item?.values?.id ? userId : 0,
  }));
  createCraneStopesDetails(
