...
function RowMaterialAutoPR() {
  ...
  const [itemRequest, setItemRequest] = useState(true);
   ...
   useEffect(() => {
    getRowData(
      `/mes/WeightBridge/GetAllQCListForPRLanding?PageNo=${pageNo}&PageSize=${pageSize}&BusinessUnitId=${selectedBusinessUnit?.value}&Status=${0}`
   );
   ...
   const headerCheckBoxHandler = (name, value) => {
      ...
      const approval = newData?.some(itm => (itm.isPRCompleted === true && !itm?.intPurchaseRequestId));
      if (approval) {
   ...
   const singleCheckBoxHandler = (name, value, index) => {
      ...
      const approval = data?.some(itm => (itm.isPRCompleted === true && !itm?.intPurchaseRequestId));
      if (approval) {
        setItemRequest(false);
      } else {
        setItemRequest(true);
   ...
   const setPositionHandler = (pageNo, pageSize, values, searchValue = '') => {
    ...
    let toDate = values?.toDate ? `&ToDate=${values?.toDate}` : ""
      getRowData(
         `/mes/WeightBridge/GetAllQCListForPRLanding?PageNo=${pageNo}&PageSize=${pageSize}&BusinessUnitId=${selectedBusinessUnit?.value}${fromDate}${toDate}&Status=${values?.status?.value || 0}&Search=${searchValue}`
      );
   };
