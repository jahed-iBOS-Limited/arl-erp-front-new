...
export default function ShippingNoteView({ id, shipmentCode, state }) {
    ...
    useEffect(() => {
        if (id) {
            ...
            getTransportStatusAndInfo(2, buId, id, setTransportStatus, setLoading);
        }
    }, [id, buId]);
    ...
    const getDeliveryShippingPrintInfoById = async (id) => {
        try {
            ...
            const res = await Axios.get(
                `/wms/Delivery/GetDeliveryPrintInfoTwo?VeichleEntryId=${id}`
            );
            if (res && res.data) {
