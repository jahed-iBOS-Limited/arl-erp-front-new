...
export default function PhysicalTestForm() {
  ...
  useEffect(() => {
    if (!location?.state?.qcTransactionHeaderId) {
      getFields(
        `/hcm/QCTest/QCParameterByBusinessUnitId?BusinessUnitId=${selectedBusinessUnit?.value}&QcTestType=PhysicalTest`
      );
    }
    getMachineList(
      `/hcm/QCTest/GetTransactionMachineNameDDL?BusinessUnitId=${location?.state?.businessUnitId || selectedBusinessUnit?.value}`
    );
  }, [selectedBusinessUnit]);

  useEffect(() => {
    if (location?.state?.qcTransactionHeaderId) {
      const data = location?.state?.row.map((item) => ({
        ...item,
        value: item?.quantity || '',
      }));
      setFields({ data });
      setModifieData({
        ...
        vrmName: location?.state?.machineId
          ? {
              value: location?.state?.machineId,
              label: location?.state?.machineName,
            }
          : '',
  ...
  const saveHandler = (values, cb) => {
    saveData(
      location?.state?.qcTransactionHeaderId
        ? `/hcm/QCTest/QCTransactionEdit`
        : `/hcm/QCTest/QCTransactionCreate`,
      makePayload({
        isEdit: location?.state?.qcTransactionHeaderId ? true : false,
        values,
        buId: selectedBusinessUnit?.value,
        userId: profileData?.userId,
        fields,
        headerId: location?.state?.qcTransactionHeaderId || 0,
      }),
  ...
  return (
    <Formik
      ...
      initialValues={
        location?.state?.qcTransactionHeaderId ? modifieData : initData
      }
