...
import { convertDecimalToTime, convertTimeIntoDecimal, getDifferenceBetweenTime } from '../../../msilProduction/meltingProduction/helper';
                     ...
                     <div className="form-group row global-form">
                        ...
                        <div className="col-lg-4">
                           <InputField
                              ...
                              onChange={e => {
                                 ...
                                 if (values?.date && values?.endTime && selectedBusinessUnit?.value !== 4) {
                        ...
                        <div className="col-lg-4">
                           <InputField
                              ...
                              type="time"
                              onChange={e => {
                                 if (!values?.date) return toast.warn('Please select date');
                                 setFieldValue('endTime', e.target.value);
                                 if (values?.date && values?.startTime && selectedBusinessUnit?.value !== 4) {
                                    let difference = getDifferenceBetweenTime(
                                       values?.date,
                                       values?.startTime,
                                       e.target.value
                        ...
                        <div className="col-lg-4">
                           <InputField
                              ...
                              label={selectedBusinessUnit?.value !== 4 ? 'Total Hours' : 'Actual Running Hours'}
                              name="totalTime"
                              type={"type"}
                           />
                        </div>
                           ...
                           <>
                                 ...
                                 <InputField
                                    ...
                                    onChange={e => {
                                       ...
                                       if (values?.date && e.target.value) {
                                          ...
                                          setFieldValue('numDuration', convertTimeIntoDecimal(breakdownDifference));
                              ...
                              >
                                 <button
                                    type="button"
                                    onClick={() => {
                                       let actualRunninHour = (((8.00) - (getActualDuration(rowData) || 0.0) || 0.0).toFixed(2));
                                       if (+actualRunninHour <= 0) {
                                          return toast.warn("Actual running hour cann't be negative or zero")
                                       }                                       
                                       addRowDataData(values, (rowData)=>{
                                          setFieldValue('totalTime', convertDecimalToTime(((8.00 - (getActualDuration(rowData) || 0.0) || 0.0))));
                                       });
                                    }}
                                    disabled={
                                       params?.id ||
                                       !values?.strBreakdownType?.label || !values?.strReasonOfStopage
                                    }
                              ...
                              <div style={{ marginTop: "20px" }}>
                                 <strong style={{ marginRight: "5px" }}> Total Breakdown Hour : {convertDecimalToTime((getActualDuration(rowData))) || ""}</strong>
                                 <strong> Actual Running Hours : { 
                                 location?.state?.tmTotalHour ? location?.state?.tmTotalHour 
                                 : convertDecimalToTime(((8.00 - (getActualDuration(rowData) || 0.0) || 0.0)))}</strong>
                              </div>
                           </>
                        ) : null}
                     </div>
                                    ...
                                    {rowData?.map((item, index) => {
                                       return (
                                          <tr key={index}>
                                             ...
                                             <td className="text-center align-middle">
                                             <OverlayTrigger
                                                overlay={<Tooltip id="delete-icon">{"Delete"}</Tooltip>}
                                              >
                                                <span>
                                                  <i
                                                    onClick={() => {
