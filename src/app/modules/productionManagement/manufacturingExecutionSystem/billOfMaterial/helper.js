...
export const getCostTypeDDL = async (setter) => {
  try {
    const res = await Axios.get(`/mes/MesDDL/GetProductionCostTypeDDL`);
    if (res.status === 200 && res?.data) {
      setter(res?.data);
    }
  } catch (error) {
...
export const getCostCenterDDL = async (accId, buId, setter) => {
  try {
    const res = await Axios.get(
      `/costmgmt/CostCenter/GetCostCenterInformation?AccountId=${accId}&BusinessUnitId=${buId}`
    );
    if (res.status === 200 && res?.data) {
      setter(
        res?.data?.map((item) => {
