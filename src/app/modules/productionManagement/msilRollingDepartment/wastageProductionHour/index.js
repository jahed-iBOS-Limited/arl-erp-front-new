...
export default function WastageProduction() {
  ...
  const rowDeleteHandler = (id, values) => {
    IConfirmModal({
      ...
      yesAlertFunc: () => {
        deleteHandler(
          `/mes/MSIL/DeleteRollingWastageAndProductionHour?headerId=${id}`,
          null,
          () => {
            getlandingData(
              `mes/MSIL/GetRollingWastageAndProductionHourLanding?FromDate=${values?.fromDate ||
  ...
  return (
    <div>
      <ITable
        link="/production-management/msil-Rolling/WastageProductionHour/create"
        title="Wastage Production Hour"
      >
        <Formik
                            ...
                            <td className="text-center">
                              {item?.tmMillRunningHour &&
                                item?.tmMillRunningHour?.split(":")?.[0] +
                                  "H"}{" "}
                              {item?.tmMillRunningHour &&
                                item?.tmMillRunningHour?.split(":")?.[1] + "M"}
                            </td>
                            <td className="text-center">
                              {item?.tmActualProductionHour &&
                                item?.tmActualProductionHour?.split(":")?.[0] +
                                  "H"}{" "}
                              {item?.tmActualProductionHour &&
                                item?.tmActualProductionHour?.split(":")?.[1] +
                                  "M"}
                            </td>
                            <td className="text-center">
                              <div className="d-flex justify-content-between">
                                <div>
                                  <IEdit
                                    onClick={() => {
                                      history.push({
                                        pathname: `/production-management/msil-Rolling/WastageProductionHour/edit/${item?.intWastageAndProductionHourHeaderId}`,
                                        state: { ...item },
                                      });
                                    }}
                                  />
