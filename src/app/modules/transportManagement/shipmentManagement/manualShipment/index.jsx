...
export default function ManualShipment() {
  ...
  useEffect(() => {
    getShippointDDL(
      `/domain/OrganizationalUnitUserPermission/GetOrganizationalUnitUserPermissionByUnitId?UserId=${
        profileData?.userId
      }&ClientId=${profileData?.accountId}&BusinessUnitId=${
      ...
      (date) => {
        ...
        getManualShipment(
          `/tms/Vehicle/GetLoadingSlip?BusinessUnitId=${selectedBusinessUnit?.value}&ShipPointId=${
            initData.shipPoint?.value
          }&fromDate=${initData.fromDate}&toDate=${initData.toDate}`
        );
      }
    );
  }, []);
  ...
  const deleteRow = (id, cb) => {
    axios
      .put(
        `/tms/Vehicle/DeleteLoadingSlipInfo?UserId=${profileData?.userId}&BusinessUnitId=${selectedBusinessUnit?.value}&LoadingSlipId=${id}`
      )
      .then((res) => {
        if (res?.status === 200) {
  ...
  const deleteHandler = (values, id) => {
    let confirmObject = {
      ...
      yesAlertFunc: async () => {
        deleteRow(id, () => {
          getManualShipment(
            `/tms/Vehicle/GetLoadingSlip?BusinessUnitId=${selectedBusinessUnit?.value}&ShipPointId=${values?.shipPoint?.value}&fromDate=${values?.fromDate}&toDate=${values?.toDate}`
          );
        });
      },
      noAlertFunc: () => {
  ...
  const { printRefCement, handleInvoicePrintCement } = DataPrintHandler();
  ...
  return (
    ...
    >
      {({
        ...
        <>
          {(manualShipmentLoader || shippointDDLloader || loading) && (
            <Loading />
          )}
          <IForm
              ...
              return (
                <div>
                  <button
                    ...
                    onClick={() => {
                      history.push(
                        '/transport-management/shipmentmanagement/manual-shipment/create'
                      );
                    }}
                  >
                    Create
              ...
              <div className="form-group  global-form row">
                ...
                <div className="col-lg-2">
                  <button
                    ...
                    disabled={
                      !values?.shipPoint || !values?.fromDate || !values?.toDate
                    }
                  >
                    View
                  </button>
                            ...
                            <tr key={index}>
                              ...
                              >
                                ...
                                <IEdit
                                  ...
                                  onClick={() => {
                                    history.push(
                                      `/transport-management/shipmentmanagement/manual-shipment/edit/${item?.intLoadingId}`
                                    );
                                  }}
                                />
                                <OverlayTrigger
              ...
              <div ref={printRefCement} className="loading-slip-report-wrapper">
                ...
                <div className="weight-report-header center mt-5 table-responsive">
                  <table className="w-100">
                    <tr>
                      <td>
                        Entry Code: {printData?.headerDTO?.strGateEntryCode}
                      </td>
                      <td>Ship Point: {printData?.headerDTO?.shipPointName}</td>
                      <td>
                        Vehicle No: {printData?.headerDTO?.strVehicleName}
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>
                        Driver Contact No :
                        {printData?.headerDTO?.driverContactNO}
                      </td>
                      <td>
                        Date: {_dateFormatter(printData?.headerDTO?.dteDate)}
                      </td>
                    </tr>
                  </table>
                  <div className="row">
          ...
          >
            <ViewManualShipment
              viewData={viewData}
              intLoadingId={viewData?.intLoadingId}
            />
          </IViewModal>
        </>
      )}
