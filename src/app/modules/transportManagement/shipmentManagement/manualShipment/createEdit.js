...
export default function ManualShipmentCreate() {
  ...
  useEffect(() => {
    if (loadingId) {
      getEditData(
        `/tms/Vehicle/GetLoadingSlipById?LoadingId=${loadingId}`,
        (data) => {
          getItemDDL(
            `/item/ItemSales/GetItemSalesByChannelAndWarehouseDDL?AccountId=${profileData?.accountId}&BUnitId=${selectedBusinessUnit?.value}&DistributionChannelId=${data?.data?.headerDTO?.intDistributionChannelId}&SalesOrgId=${data?.data?.headerDTO?.intSalesOrgId}`
          );
    ...
    getSalesBU(
      ...
      (data) => {
        ...
        getSalesOrg(
          `/oms/SalesOrder/GetSODDLbySBUId?AccountId=${profileData?.accountId}&BusinessUnitId=${selectedBusinessUnit?.value}&SBUId=${data[0]?.value}`
        );
        getDistributionChannel(
          `/oms/SalesOrder/GetDistributionChannelDDLBySBUId?AccountId=${profileData?.accountId}&BusinessUnitId=${selectedBusinessUnit?.value}&SBUId=${data[0]?.value}`
        );
      }
    );
    getUom(
  ...
  const saveHandler = (values, cb) => {
    ...
    const payload = {
      headerDTO: {
        ...
        intVehicleId: loadingId
          ? modifyData?.intVehicleId
          : values?.entryCode?.intvehicleid,
        strVehicleName: loadingId
          ? modifyData?.strVehicleName
          : values?.entryCode?.vehicleNo,
        intActionBy: profileData?.userId,
        intGateEntryId: loadingId
          ? modifyData?.intGateEntryId
    ...
    loadingId
      ? updateLoadingSlip(payload, setLoader, cb)
            ...
            <Form>
              <div className="form-group  global-form row">
                ...
                <div className="col-lg-2">
                  ...
                  <SearchAsyncSelect
                    ...
                    loadOptions={(v) => {
                      ...
                      return axios
                        .get(
                          `/mes/MSIL/GetAllMSIL?PartName=EntryCodeDDL&BusinessUnitId=${selectedBusinessUnit?.value}&search=${v}`
                        )
                        .then((res) => {
                          return res?.data;
                        })
                ...
                <div className="col-lg-2">
                  <button
                    onClick={() => {
                      let isItemAlreadyAdded = rowData.find(
