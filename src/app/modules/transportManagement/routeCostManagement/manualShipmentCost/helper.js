...
export const CreateFuelConstInfo_api = async (payload) => {
  try {
    const res = await Axios.post(
      `/tms/FuelCostInfo/CreateFuelConstInfoManual`,
      payload
    );
    if (res.status === 200 && res?.data) {
...
export const editShipment = async (payload, setDisabled) => {
  ...
  try {
    const res = await Axios.post(
      `/tms/ShipmentStandardCost/CreateShipmentStandardCostManual`,
      payload
    );
    if (res.status === 200 && res?.data) {
    ...
    if (res.status === 200 && res?.data) {
      ...
      const newObj = {
        ...
        vehicleInDate: objHeader?.vehicleInDate
          ...
          : '',
        vehicleInTime: objHeader?.vehicleInDate ? vehicleInTime : '',
      };
      setter(newObj);
...
export const getGridData = async (
  ...
  try {
    setLoading(true);
    const res = await Axios.get(
      `/tms/ShipmentStandardCost/ShipmentStandardCostPaginationManual?accountId=${accId}&businessUnitId=${buId}&FromDate=${fromDate}&ToDate=${toDate}&isBillSubmited=${rtId}&PageNo=${pageNo}&PageSize=${pageSize}&viewOrder=desc`
    );

    paginationSetter(res?.data);
...
export const getRouteNameDDL = async (accId, buId, setter) => {
  try {
    const res = await Axios.get(
