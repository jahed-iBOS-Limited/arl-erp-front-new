          ...
          >
            ...
            <tbody>
              {printData?.map((item, index) => {
                ...
                const totalCost =
                  totalRouteCost + item?.otherExpanse + item?.daAmount;
                const perKMCost = totalRouteCost / (item?.totalKM || 1);
                const personalCostDeduction =
                  ((totalPersonalKM - item?.numCeilingKM > 0 &&
                    totalPersonalKM - item?.numCeilingKM) ||
                    0) * (perKMCost || 0);
                const netPayable =
                ...
                grandKMPL +=
                  (item?.fuelCash + item?.fuelCredit + item?.numTollAmount) /
                  item?.totalKM;
