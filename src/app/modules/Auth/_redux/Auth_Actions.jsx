...
export const saveChatInfoAction =
  (email, name, accountId, userId) => (dispatch) => {
...
export const otpSendAndVerify = async (
  ...
  axios({
    method: 'get',
    url: `/fino/CommonFino/CheckTwoFactorApproval?OtpType=${type}&intUnitId=4&strTransectionType=Login&intTransectionId=${userId}&strCode=${'123'}&intActionById=${userId}&strOTP=${
      otp || ''
    }&CancelType=1`,
...
export const Login =
  (email, password, setLoading, cb, setIsOtp, isSkipOtp, setUserId, setToken) =>
  (dispatch) => {
      ...
      menuItem.subs = menuItem?.subs?.filter((subItem) => {
        ...
        if (subItem?.id === 53) {
          ...
          subItem.nestedSubs = subItem?.nestedSubs?.filter((subItem) => {
            ...
            if (
              [
                670, 338, 299, 289, 288, 285, 284, 283, 282, 280, 274, 272, 270,
                268,
              ].includes(subItem?.id)
            )
              return false;
