...
export default function StockTransferPOCreateForm({
  ...
  const loadUserList = (v) => {
    ...
    return axios
      .get(
        `/procurement/PurchaseOrder/GetPOReferenceNoWiseItemDDLWithoutRefItemSearch?searchTerm=${v}&OrderTypeId=${4}&AccountId=${
          profileData?.accountId
        }&BusinessUnitId=${selectedBusinessUnit?.value}&SbuId=${
          location?.state?.sbu?.value
        }&PurchaseOrgId=${location?.state?.purchaseOrg?.value}&PlantId=${
          location?.state?.plant?.value
        }&WearhouseId=${
          location?.state?.warehouse?.value
