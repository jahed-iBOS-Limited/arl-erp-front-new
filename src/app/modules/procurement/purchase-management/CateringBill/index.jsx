...
export default function CateringBill() {
  ...
  const getLandingData = (values) => {
    const strSupliarName =
      values?.supplierName?.label === 'All' || !values?.supplierName?.label
        ? ''
  ...
  const onUpdateHandler = (values, payload, setFieldValue) => {
    onSubmitHandler(
      '/procurement/CateringBill/UpdateAutoPurchaseOrderForMeal',
      payload,
      () => {
      ...
      {({
        ...
        <>
          {(loading ||
            ...
            singleDataLoading ||
            createPOLoading ||
            inventoryReceiveLoading) && <Loading />}
          <IForm title="Catering Bill" isHiddenReset isHiddenBack isHiddenSave>
            <Form>
              <>
                <div className="form-group  global-form row">
                  <div className="col-lg-3">
                    <NewSelect
                      name="businessUnit"
                      options={
                        [{ value: 0, label: 'All' }, ...businessUnitList] || []
                      }
                  ...
                  <div className="col-lg-3">
                    <NewSelect
                      ...
                      options={
                        [
                          { value: 0, label: 'All' },
                          { value: 1, label: 'Champak' },
                          { value: 2, label: 'R B Food Catering' },
                          { value: 3, label: 'AusBD Kitchen Ltd.' },
                        ] || []
                      }
                ...
                {gridData?.length > 0 && (
                  <div className="text-right my-2">
                    ...
                    <span className="ml-2">
                      <button
                        disabled={!selectedGridData?.length}
                        onClick={() => {
                  ...
                  <div className="table-responsive">
                    <table className="table table-striped mt-2 table-bordered bj-table bj-table-landing">
                      ...
                      <tbody>
                          ...
                          <tr key={item.mealConsumeCountId}>
                            <td>
                              <input
                                ...
                                onChange={(e) => {
                                  const checked = e.target.checked;
                                  const modifyData = gridData?.map((itm) =>
                                    itm.mealConsumeCountId ===
                                ...
                                {!item?.purchaseOrderNo && (
                                  <span
                                    ...
                                    onClick={() => {
                                      ...
                                      getSingleData(
                                        `/procurement/CateringBill/GetAutoPurchaseOrderForMealById?MealConsumeCountId=${item?.mealConsumeCountId}`,
                                        (data) => {
                                          IConfirmModal({
                                            message:
                                              'Are you sure you want to create PO?',
                                            title: 'Create PO',
                                            yesAlertFunc: () => {
                                              ...
                                              const year =
                                                fourteenDaysLater.getFullYear();
                                              const month = String(
                                                fourteenDaysLater.getMonth() + 1
                                              ).padStart(2, '0'); // Months are 0-indexed
                                              const day = String(
                                                fourteenDaysLater.getDate()
                                              ).padStart(2, '0');
                                                ...
                                                objRowListDTO: [
                                                  {
                                                    ...
                                                    controllingUnitId:
                                                      data?.controllingUnitId,
                                                    bomId: 0,
                                                    controllingUnitName:
                                                      data?.controllingUnitName,
                                                    costCenterId:
                                                      data?.costCenterId,
                                                    costCenterName:
                                                      data?.costCenterName,
                        ...
                        <tr>
                          <td colSpan="5" className="text-right text-bold">
                            Total
                          </td>
