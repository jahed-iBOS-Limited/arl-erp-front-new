...
export default function CateringBill() {
  ...
  const getLandingData = (values) => {
    getGridData(
      `/procurement/CateringBill/GetAutoPurchaseOrderForMeal?AccountId=${
        profileData?.accountId
      }&BusinessUnitId=${values?.businessUnit?.value || 0}&MonthId=${
        values?.monthYear?.split('-')[1]
      }&YearId=${values?.monthYear?.split('-')[0]}&SupplierName=${values
        ?.supplierName?.label || ''}`,
    );
  };
