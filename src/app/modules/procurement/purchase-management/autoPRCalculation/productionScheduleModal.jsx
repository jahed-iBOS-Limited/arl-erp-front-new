...
export default function ProductionScheduleModal({ buId, fromData, toDate }) {
  const [productionScheduleData, getProductionScheduleData, loading] =
    useAxiosGet();

  useEffect(() => {
    getProductionScheduleData(
      `/mes/ProductionEntry/GetDetailFromProductionSchedule?intBusinessUnitId=${buId}&isForecast=true&fromDate=${fromData}&toDate=${toDate}`
    );
  }, []);
                        ...
                        <tbody>
                          {productionScheduleData?.map((item, index) => (
                            <tr key={index}>
                              ...
                              <td className="text-center">
                                {item.numBomQty.toFixed(6)}
                              </td>
                              <td className="text-center">
                                {item.numTotalQty.toFixed(2)}
                              </td>
                            </tr>
                          ))}
                        </tbody>
