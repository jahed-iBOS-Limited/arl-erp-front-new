...
export default function AssetStandardPOCreateForm({
    ...
    if (values?.isAllItem) {
      ...
      if (arr?.length > 0) {
        ...
        const newData = {
          ...
          item: { ...values?.item },
          desc:
            values?.item?.quotationEntryRemarks === ''
  ...
  const basicAmountFromExcelToUI = (excelDataForAmount) => {
    excelDataForAmount.forEach((excelElement, excelIndex) => {
      rowDto.forEach((rowElement, rowIndex) => {
        if (
          excelElement?.Item.trim().toLowerCase() ===
          rowElement?.item?.itemName.trim().toLowerCase()
        ) {
  ...
  const getRunTimeGrossDiscount = (discountData) => {
    ...
    const totalBasic = discountData.reduce(
      (acc, cur) => acc + +cur?.netValue,
      0,
    );
  ...
  const totalValueWithoutDiscountAndVat = rowDto?.reduce(
    (acc, cur) => acc + cur?.netValue,
    0,
  );
  console.log(
    'totalValueWithoutDiscoutnAndVat',
    totalValueWithoutDiscountAndVat,
  );
  return (
    <>
      <Formik
                ...
                <div className="form-group row">
                  ...
                  <div className="col-lg-2">
                    <IInput
                      ...
                      label={
                        location?.state?.refType?.value === 4
                          ? `Gross Discount (${
                              values?.referenceNo?.numDiscountPercentage
                                ? values?.referenceNo?.numDiscountPercentage
                                : ''
                            }%)`
                          : 'Gross Discount'
                      ...
                      dependencyFunc={(
                        ...
                        if (valueOption) {
                          ...
                          if (location?.state?.refType?.value === 4) {
                            setFieldValue(
                              'othersCharge',
                              valueOption?.numOthersCost,
                            );
                            setFieldValue(
                              'freight',
                              valueOption?.numTransportCost,
                            );
