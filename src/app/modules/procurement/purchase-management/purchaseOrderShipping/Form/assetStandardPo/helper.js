...
export const getRefNoDdlForStandradPo = async (
  ...
  try {
    const res = await Axios.get(
      ...
      //`/procurement/ShipRequestForQuotation/GetRFQShipForSupplierDDL?AccountId=${accId}&BusinessUnitId=${buId}&SBUId=${sbuId}&BusinessPartnerId=${buPartnerId}`
      // `/procurement/PurchaseOrderReferenceNo/GetStandardPOreferenceNoDDL?accountId=${accId}&businessUnitId=${buId}&sbuId=${sbuId}&purchaseOrgId=${orgId}&plantId=${plantId}&warehouseId=${whId}&refType=${refTyp}` //Revious DDL
    );
