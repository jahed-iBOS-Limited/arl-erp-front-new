export const rowDtoDynamicHandler = (name, value, sl, rowDto, setRowDto) => {
  ...
  if (name === 'orderQty') {
    const netValue =
      value * _sl.basicPrice + ((value * _sl.basicPrice) / 100) * _sl?.vat;
    ...
    const totalBasic = rowDto.reduce((acc, cur) => acc + +cur?.netValue, 0);

    const discount = (totalBasic * rowDto[0]?.discountPercentage) / 100;
    _sl.discount = discount || 0;
  } else if (name === 'basicPrice') {
    const netValue =
      value * _sl.orderQty + ((value * _sl.orderQty) / 100) * _sl?.vat;
    _sl.netValue = netValue || 0;
    ...
    const netValue =
      _sl.orderQty * _sl.basicPrice +
      ((_sl.orderQty * _sl.basicPrice) / 100) * value;
    _sl.netValue = netValue || 0;
    ...
    const netValue =
      _sl.orderQty * _sl.basicPrice +
      ((_sl.orderQty * _sl.basicPrice) / 100) * _sl.vat;
