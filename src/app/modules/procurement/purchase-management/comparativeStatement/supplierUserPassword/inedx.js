...
const SupplierUserPassword = () => {
  ...
  const { profileData } = useSelector((state) => {
   ...
   useEffect(()=>{
      getRowData(`/hcm/Report/GetSupplierCredential?AccountId=${profileData?.accountId}&BusinessUnitId=102&PageNo=${pageNo}&PageSize=${pageSize}&viewOrder=desc`)
  ...
  const setPositionHandler = (pageNo, pageSize, searchValue) => {
   getRowData(
     `/hcm/Report/GetSupplierCredential?AccountId=${profileData?.accountId}&BusinessUnitId=102&Search=${searchValue || ""}&PageNo=${pageNo}&PageSize=${pageSize}&viewOrder=desc`
   );
 };
