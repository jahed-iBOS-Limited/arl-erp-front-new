...
export function ComparativeStatementShippingPages() {
  ...
  for (let i = 0; i < userRole.length; i++) {
    if (userRole[i]?.intFeatureId === 1152) {
      //For Live
      rfqPermission = userRole[i];
    }
    if (userRole[i]?.intFeatureId === 1153) {
      //For Live
      quotationEntryPermission = userRole[i];
    }
    if (userRole[i]?.intFeatureId === 1154) {
      //For Live
      csPermisssion = userRole[i];
    }
    if (userRole[i]?.intFeatureId === 1243) {
      //For Live
      supplierCredintial = userRole[i];
    }
    if (userRole[i]?.intFeatureId === 1265) {
      //For Live
  ...
  return (
    <Switch>
      ...
      <ContentRoute
        ...
        component={
          csPermisssion?.isView
            ? ShippingComparativeStatement
            : NotPermittedPage
        }
      />
      <ContentRoute
        ...
        component={
          quotationEntryPermission?.isCreate
            ? ShippingNegotiationCreate
            : NotPermittedPage
        }
      />
      <ContentRoute
      ...
      <ContentRoute
        ...
        component={
          quotationEntryPermission?.isView
            ? ShippingQuotationEntry
            : NotPermittedPage
        }
      />
      <ContentRoute
      ...
      <ContentRoute
        ...
        component={
          rfqPermission?.isCreate ? ShippingRFQCreate : NotPermittedPage
        }
      />
      <ContentRoute
        from="/mngProcurement/comparative-statement/shipping-rfq"
        component={
          rfqPermission?.isView ? ShippingRFQLanding : NotPermittedPage
        }
      />
      <ContentRoute
        from="/mngProcurement/comparative-statement/supplier-credential"
        component={
          supplierCredintial?.isView ? SupplierUserPassword : NotPermittedPage
        }
      />
      <ContentRoute
        from="/mngProcurement/comparative-statement/quotation-history-report"
        component={
          quotationHistory?.isView ? QuotationHistoryReport : NotPermittedPage
        }
      />
      <ContentRoute
