...
export default function ShippingQuotationCreate() {
  ...
  const saveHandler = async (values, cb) => {
    ...
    const payload = {
      objHeader: {
        intPartnerRfqid: values?.supplierName?.intPartnerRfqid,
        intRequestForQuotationId: rowDto?.objHeader?.intRequestForQuotationId,
          ...
          initialValues={{
            ...
            supplierName:
              supplierNameDDL?.length === 1
                ? {
                    ...
                    intPartnerRfqid: supplierNameDDL[0]?.intPartnerRfqid,
            ...
            supplierContactNo:
              supplierNameDDL?.length === 1
                ...
                : '',
            supplierEmail:
              supplierNameDDL?.length === 1 ? supplierNameDDL[0]?.strEmail : '',
          }}
                ...
                <div className="form-group  global-form">
                  <div className="row">
                    <div className="col-lg-3">
                      RFQ Code: {rowDto?.objHeader?.strRequestForQuotationCode}
                    </div>
                    <div className="col-lg-3">
                      RFQ Date: {_dateFormatter(rowDto?.objHeader?.dteRfqdate)}
                    </div>
                    <div className="col-lg-3">
                      Currency: {rowDto?.objHeader?.strCurrencyCode}
                    </div>
                    <div className="col-lg-3">
                      Delivery Address: {rowDto?.objHeader?.strDeliveryAddress}
                    </div>
                    <div className="col-lg-3">
                      Quotation Start Date:{' '}
                      {`${
                        rowDto?.objHeader?.quotationStartDateTime?.split('T')[0]
                      } / ${
                        rowDto?.objHeader?.quotationStartDateTime?.split('T')[1]
                      }`}
                    </div>
                    <div className="col-lg-3">
                      Quotation End Date:{' '}
                      {`${
                        rowDto?.objHeader?.quotationEndDateTime?.split('T')[0]
                      } / ${
                        rowDto?.objHeader?.quotationEndDateTime?.split('T')[1]
                      }`}
                    </div>
                  </div>
                  <div className="row">
                    ...
                    <div className="col-lg-3">
                      <InputField
                        ...
                        onChange={(e) => {
                          if (e.target.value > 0) {
                            setFieldValue('transportCost', e.target.value);
                          } else {
                    ...
                    <div className="col-lg-3">
                      <InputField
                        ...
                        onChange={(e) => {
                          if (e.target.value > 0) {
                            setFieldValue('othersCost', e.target.value);
                          } else {
                      ...
                      <InputField
                        ...
                        onChange={(e) => {
                          if (e.target.value > 0) {
                            ...
                            getPercentageValue(e.target.value, subTotal);
                          } else {
