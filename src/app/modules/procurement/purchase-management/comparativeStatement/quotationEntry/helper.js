...
export const getSupplierNameDDLAction = async (
  accId,
  buId,
  rfqId,
  userId,
  ...
  try {
    const res = await axios.get(
      `/procurement/ShipRequestForQuotation/GetQuotationEntrySupplierListDDL?AccountId=${accId}&BusinessUnitId=${buId}&RequestForQuotationId=${rfqId}&UserId=${userId}`
    );
...
export const createQuotationEntry = async (payload, cb, setLoading) => {
  ...
  try {
    await axios.put(
      `/procurement/ShipRequestForQuotation/CreateRFQEntryPageShip`,
      payload
    );
...
export const updateSupplierPasss = async (payload, cb) => {
  try {
    await axios.put(`/domain/Information/Basic`, payload);
