...
export default function ShippingRFQCreate() {
  ...
  useEffect(() => {
    if (id) {
      getSingleData(
        ...
        setRfqItemDDL,
        setLoading,
        location?.rfqSingleData?.referenceCode,
        profileData?.accountId,
        selectedBusinessUnit?.value
      );
    }
  ...
  const saveHandler = async (values, cb) => {
    ...
    if (id) {
      ...
      saveData(
        `/procurement/ShipRequestForQuotation/CreateRequestForQuotationShip`,
        payload,
        cb,
        true
      );
    ...
    if (values?.isAllItem) {
      ...
      if (isExists?.length > 0) {
        ...
        const newData = {
          ...
          uomDDL: {
            label: values?.itemName?.strUoMname,
            value: values?.itemName?.intUoMid,
          },
  ...
  const setterTwo = (values) => {
    if (
      isUniq('intBusinessPartnerId', values?.supplierName?.value, rowDtoTwo)
    ) {
      let obj = {
  ...
  const removerTwo = (intBusinessPartnerId) => {
    const filterArr = rowDtoTwo.filter(
      (itm) => itm.intBusinessPartnerId !== intBusinessPartnerId
    );
                  ...
                  <div className="row">
                    <div className="col-lg-3">
                      <NewSelect
                        ...
                        isDisabled={
                          modifyData?.strStatus === 'Open' ||
                          modifyData?.strStatus === 'Closed'
                        }
                      />
                    </div>
                    <div className="col-lg-3">
