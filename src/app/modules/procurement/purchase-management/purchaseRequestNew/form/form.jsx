   ...
   const loadUserList = v => {
      ...
      return Axios.get(
         `/wms/ItemPlantWarehouse/GetItemPlantWarehouseForPurchaseRequestSearchDDL?accountId=${
            profileData?.accountId
         }&businessUnitId=${selectedBusinessUnit?.value}&plantId=${location
            ?.state?.plant?.value || initData?.plantId}&whId=${location?.state
            ?.wh?.value ||
            initData?.warehouseId}&purchaseOrganizationId=${location?.state?.po
            ?.value ||
            initData?.purchaseOrganizationId}&typeId=${itemTypeId}&searchTerm=${v}${standardItemTypeId}${itemCategoryId}`
      ).then(res => {
