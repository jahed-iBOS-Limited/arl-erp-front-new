...
export function TerritorySalesForceConfig({ history }) {
  const uIEvents = {
    openEditPage: (id) => {
      history.push(
        `/sales-management/configuration/territorysalesforceconfig/edit/${id}`
      );
    },
    openViewDialog: (id) => {
      history.push(
        `/sales-management/configuration/territorysalesforceconfig/view/${id}`
      );
    },
  };
      ...
      <Route path="/sales-management/configuration/territorysalesforceconfig/view/:id">
        {({ history, match }) => (
          <ViewForm
            ...
            onHide={() => {
              history.push(
                '/sales-management/configuration/territorysalesforceconfig'
              );
            }}
          />
        )}
