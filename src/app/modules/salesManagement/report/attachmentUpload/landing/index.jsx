...
const initData = {
  type: { value: 1, label: 'BUET Test Report' },
  channel: '',
  customer: '',
};
...
function AttachmentUpload() {
  ...
  const [
    customerTargetPolicy,
    getCustomerTargetPolicy,
    getCustomerTargetPolicyLoading,
    setCustomerTargetPolicyData,
  ] = useAxiosGet();
  const [, updateCustomerRowAttachement, updateCustomerRowAttachementLoading] =
    useAxiosPost();

  useEffect(() => {
    commonGridDataFunc(initData, pageNo, pageSize);
    getType(
      `/partner/BusinessPartnerBasicInfo/GetPolicyAttachmentType`,
      (data) => {
  ...
  const handleCustomerTargetPolicyLanding = (values, pageNo, pageSize) => {
    getCustomerTargetPolicy(
      `/partner/BusinessPartnerBasicInfo/GetBusinessPartnerPolicyPagination?search=${values?.customer.length > 0 ? values?.customer : ''}&businessUnitId=${buId}&channelId=${values?.channel?.value}&attachmentTypeId=${values?.type?.value}&pageNo=${pageNo}&pageSize=${pageSize}`
    );
  };
  ...
  return (
    <>
      {(loading ||
        updateCustomerRowAttachementLoading ||
        getCustomerTargetPolicyLoading) && <Loading />}
      <ICustomCard
          ...
          {({ values, setFieldValue, errors, touched }) => (
            <>
              <Form>
                    ...
                    <>
                      ...
                      <div className="col-lg-3 row">
                        <IButton
                          ...
                          colSize={6}
                        />
                        {customerTargetPolicy?.data?.filter(
                          ...
                          <IButton
                            onClick={() => {
                              if (
                                ...
                                updateCustomerRowAttachement(
                                  `/partner/BusinessPartnerBasicInfo/CreateBusinessPartnerPolicy`,
                                  customerTargetPolicy?.data,
                                  () => {
                                    commonGridDataFunc(
                                      values,
                                      pageNo,
                ...
                {values?.type?.value === 3 &&
                  customerTargetPolicy?.data?.length > 0 && (
                    <div className="table-responsive">
                      <table className="table table-striped table-bordered bj-table bj-table-landing">
                        <thead>
                          <tr>
                            <th>
                              <input
                                type="checkbox"
                                checked={customerTargetPolicy?.data?.every(
                                  (item) => item.isSelected
                                )}
                                onChange={(e) => {
                                  const checked = e.target.checked;
                                  const updateCustomerTargetPolicy =
                                    customerTargetPolicy?.data.map((item) => {
                                      return {
                                        ...item,
                                        isSelected: checked,
                                      };
                                  ...
                                  setCustomerTargetPolicyData((prevValue) => {
                                    return {
                                      ...prevValue,
                                      data: updateCustomerTargetPolicy,
                                    };
                                  });
                                }}
                              />
