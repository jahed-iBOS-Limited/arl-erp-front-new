    ...
    if (res?.status === 200 && res?.data) {
      ...
      result.forEach(({ AreaName, ...otherProps }) => {
        if (AreaName in dataByAreaCategory) {
          dataByAreaCategory[AreaName].value.push({
            ...otherProps,
            AreaName: AreaName,
          });
          dataByAreaCategory[AreaName].RegionName = otherProps?.RegionName;
          dataByAreaCategory[AreaName].totalCreditLimit +=
            otherProps?.monCreditLimit || 0;
          dataByAreaCategory[AreaName].totalDebit += otherProps?.monDebit || 0;
          dataByAreaCategory[AreaName].totalCollection +=
            otherProps?.monCollection || 0;
          dataByAreaCategory[AreaName].totalOutstanding +=
            otherProps?.monOutstanding > 0 ? otherProps?.monOutstanding : 0;
        } else {
      ...
      finalAreaData.forEach(({ RegionName, ...otherProps }) => {
        if (RegionName in dataByRegionCategory) {
          dataByRegionCategory[RegionName].value = [
            ...dataByRegionCategory[RegionName].value,
            ...otherProps?.value,
          ];
          dataByRegionCategory[RegionName].AreaName = otherProps?.AreaName;
          dataByRegionCategory[RegionName].totalCreditLimit +=
            otherProps?.value[otherProps?.value?.length - 1].totalCreditLimit ||
            0;
          dataByRegionCategory[RegionName].totalDebit +=
            otherProps?.value[otherProps?.value?.length - 1].totalDebit || 0;
          dataByRegionCategory[RegionName].totalCollection +=
            otherProps?.value[otherProps?.value?.length - 1].totalCollection ||
            0;
          dataByRegionCategory[RegionName].totalOutstanding +=
            otherProps?.value[otherProps?.value?.length - 1].totalOutstanding ||
            0;
        } else {
          dataByRegionCategory[RegionName] = {
            ...
            totalCreditLimit:
              otherProps?.value[otherProps?.value?.length - 1]
                .totalCreditLimit || 0,
            totalDebit:
              otherProps?.value[otherProps?.value?.length - 1].totalDebit || 0,
            totalCollection:
