...
export const getCheckpostListPermissionByUser = async (
  ...
  try {
    const res = await Axios.get(
      `/domain/OrganizationalUnitUserPermission/GetCheckpostListPermissionByUser?UserId=${userId}&ClientId=${accId}&UnitId=${buId}`
    );
    if (res.status === 200 && res?.data) {
...
export const getItemRequestGridData = async (
  ...
  try {
    setLoading(true);
    const res = await Axios.get(
      `/tms/CheckpostVehicleInOut/GetCheckpostVehicleInOutByCheckpostIdLanding?CheckpostId=${chekcPostId}&AccountId=${accId}&VehicleStatusId=${vehicleStsId}&viewOrder=desc&PageNo=${pageNo}&PageSize=${pageSize}`
    );
    if (res.status === 200 && res?.data) {
      setLoading(false);
