...
export default function AssetCheckInOut() {
  const { profileData, businessUnitList } = useSelector(
  ...
  const getReportData = (values) => {
    let strStatus =
      values?.status?.value === null ? '' : `&Status=${values?.status?.value}`;
    getGridData(
    ...
    >
      {({ handleSubmit, setFieldValue, values }) => (
        <>
          {loading && <Loading />}
          <IForm
            title="Asset Check-In/Out Report"
            isHiddenReset
            isHiddenBack
            isHiddenSave
          >
                    ...
                    <tbody>
                      {gridData.map((item, index) => (
                        <tr key={index}>
                          ...
                          <td className="text-center">
                            {_dateFormatter(item?.dteAcquisitionDate)}
                          </td>
