...
export const savesparePartsData = async (data, rowId, setsparePartsData) => {
  try {
    const res = await Axios.post(`/asset/MntItemTask/CreateMntItemTask`, data);
    if (res.status === 200) {
...
export const savesubserviceCostData = async (data, rowId, setsubTaskData) => {
  try {
    const res = await Axios.post(`/asset/MntTask/CreateMntSubTask`, data);
    if (res.status === 200) {
      toast.success(res?.message);
      const newData = await Axios.get(
...
export const saveItemTaskforEdit = async (data, rowId, setter) => {
  try {
    const res = await Axios.put(`/asset/MntItemTask/UpdateMntItemTask`, data);
    if (res.status === 200) {
...
export const saveSubTaskforEdit = async (data, cb) => {
  try {
    const res = await Axios.put(`/asset/MntTask/UpdateMntSubTask`, data);
    if (res.status === 200) {
      toast.success(res?.message || 'Submitted successfully');
      //cb()
    }
...
export const getInventoryCurrentBalance = async ({
  whId,
  buId,
  itemId,
  setFieldValue,
  ...
  try {
    const res = await Axios.get(
      `/asset/DetalisView/GetInventoryCurrentBalance?BusinessUnitId=${buId}&WarehouseId=${whId}&ItemId=${itemId}`
    );
    if (res.status === 200 && res?.data) {
