...
export const getActivityByRenewalServiceId = async (
  ...
  try {
    const res = await Axios.get(
      `/asset/DetalisView/GeAttributeByRenewalServiceId?ServiceId=${serviceId}&BrtaVehicelTypeId=${
        brtaId || 0
      }&assetId=${assetId}`
    );
    if (res.status === 200 && res?.data) {
...
export const getGridData = async ({
  ...
  try {
    const res = await Axios.get(
      ...
      }&AccountId=${accId}&UnitId=${buId}&PlantId=${0}&RenewalServiceId=${
        values?.renewalType?.value || 0
      }&AssetId=${values?.vehicleNo?.value || 0}${FromDate}${ToDate}&statusId=${
        values?.status?.value || 0
