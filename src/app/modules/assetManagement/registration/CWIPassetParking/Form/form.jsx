...
export default function _Form({
  ...
  const loadItemList = (v) => {
    //  if (v?.length < 3) return []
    return Axios.get(
      `/asset/DropDown/GetAssetItemListByWhId?AccountId=${profileData?.accountId}&UnitId=${selectedBusinessUnit?.value}&PlantId=${currentRowData?.intPlantId}&WHId=${currentRowData?.intWarehouseId}&searchTearm=${v}`
    ).then((res) => {
      const updateList = res?.data.map((item) => ({
        ...item,
      }));
