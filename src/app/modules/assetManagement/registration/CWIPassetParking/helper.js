...
export const getGridData = async (
  ...
  try {
    setLoading(true);
    const res = await Axios.get(
      `/asset/LandingView/GetAssetList?AccountId=${accId}&UnitId=${buId}&SbuId=${sbuId}&PlantId=${plId}&WHId=${whId}&search=${searchTerm}&PageNo=${pageNo}&PageSize=${pageSize}&viewOrder=desc`
    );
    if (res.status === 200 && res?.data) {
      setter(res?.data);
      setLoading(false);
...
export const getAssetListByUnit = async ({
  buId,
  whId,
  setter,
  setLoading,
...
export const saveAssetForData = async (data, cb, setDisabled) => {
  try {
    setDisabled(true);
    const res = await Axios.post(`/asset/Asset/CreateAsset`, data);
    if (res.status === 200) {
