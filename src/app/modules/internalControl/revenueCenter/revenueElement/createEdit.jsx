...
export default function RevenueElementCreateEdit() {
  const [controllingDDL, getControllingDDL, controllingDDLloader] =
    useAxiosGet();
  const [, getSingleDataForEdit, getSingleDataForEditLoader] = useAxiosGet();
  const [revenueCenterDDL, getRevenueCenterDDL, revenueCenterDDLloader] =
    useAxiosGet();
  ...
  const saveHandler = (values, cb) => {
    ...
    if (id) {
      ...
      saveData(
        '/costmgmt/Revenue/CreateRevenueElement',
        createPayload,
        cb,
        true
      );
  ...
  useEffect(() => {
    if (id) {
      getSingleDataForEdit(
        `/costmgmt/Revenue/GetRevenueElementById?Id=${id}`,
        (data) => {
          console.log('data', data);
          getRevenueCenterDDL(
            `/costmgmt/Revenue/GetRevenueCenterDDL?accountId=${
    ...
    getControllingDDL(
      `/costmgmt/ControllingUnit/GetControllingUnitDDL?AccountId=${profileData?.accountId}&BusinessUnitId=${selectedBusinessUnit?.value}`
    );
    getGeneralLedgerDDL(
      `/domain/BusinessUnitGeneralLedger/GetGeneralLedgerDDL?AccountId=${
        profileData?.accountId
      }&BusinessUnitId=${selectedBusinessUnit?.value}&AccountGroupId=12`,
      (data) => {
  ...
  return (
    ...
    >
      {({
        ...
        <>
          {(generalLedgerDDLLoader ||
            controllingDDLloader ||
            revenueCenterDDLloader ||
            saveDataLaoder ||
            getSingleDataForEditLoader) && <Loading />}
          <IForm
            title={id ? 'Edit Revenue Element' : 'Create Revenue Element'}
