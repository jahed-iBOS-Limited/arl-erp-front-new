  ...
  return (
    <>
      <Formik
        ...
        initialValues={{
          ...initData,
          transactionDate: location?.state?.dteTransaction
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (!values?.instrumentType)
                              return toast.warn('instrument Type is required');
                            if (!values?.instrumentNo)
                              return toast.warn('instrument No is required');
                            if (!values?.instrumentDate)
                              return toast.warn('Instrument Date is required');
                            if (!values?.headerNarration)
                              return toast.warn('Header Narration is required');
                            if (jorunalType === 4) {
                              if (!values?.receiveFrom)
                                return toast.warn('receive From is required');
                              if (!values?.placingDate)
                                return toast.warn('placing Date is required');
                            } else if (jorunalType === 5) {
                              if (!values?.paidTo)
